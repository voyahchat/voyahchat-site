...
test('CSSBuilder.saveArtifacts() - should create correct files', async (t) => {
    const dir = new TestDir();
    ...
    const siteHashedPath = path.join(dir.getSite(), `${hash}.css`);
    ...
    t.true(siteFileExists, 'Site hashed CSS file should exist');
...
test('CSSBuilder.saveArtifacts() - should clean up after build failure', async (t) => {
    ...
    fs.writeFile = async (filePath, data) => {
        // Fail only when writing the CSS file to site directory
        if (filePath.toString().includes(dir.getSite()) && filePath.toString().endsWith('.css')) {
