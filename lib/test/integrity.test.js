...
// ============================================================================
// BUILD PREREQUISITE HOOK
// ============================================================================

test.before('Ensure build exists for integration tests', (t) => {
    ...
    if (!fs.existsSync(buildHtmlPath) || !fs.existsSync(siteIndexPath)) {
        ...
        try {
            ...
            if (!fs.existsSync(buildHtmlPath) || !fs.existsSync(siteIndexPath)) {
                t.fail('Build completed but required artifacts are still missing');
            }
        } catch (error) {
...
test('link integrity - all markdown links point to existing files and anchors', async (t) => {
    ...
    const contentDir = path.join(process.cwd(), 'external', 'voyahchat-content');
...
test('link integrity - all HTML links point to existing URLs and anchors', async (t) => {
    ...
    for (const [filename] of Object.entries(buildHtml)) {
        ...
        // Find all internal links: href="/path" or href="/path#anchor"
        ...
        while ((match = linkRegex.exec(content)) !== null) {
            ...
            // Skip external links, anchors only, and non-HTML files
            ...
            if (anchor) {
                ...
                if (!mdFile) {
                    continue; // Already reported as broken URL
                }
                ...
                while ((textMatch = headingTextRegex.exec(targetContent)) !== null) {
                    ...
                    const hierarchicalId = textMatch[2];
                ...
                const mappedAnchor = anchorMapping[anchor] || anchor;

                if (!anchors.includes(mappedAnchor)) {
                    // Debug output for troubleshooting
                    console.log(`DEBUG: Looking for anchor '#${anchor}' in ${htmlFilename}`);
                    console.log(`DEBUG: Mapped to: '#${mappedAnchor}'`);
                    const anchorPreview = anchors.slice(0, 10).join(', ');
                    const moreText = anchors.length > 10 ? '...' : '';
                    console.log(`DEBUG: Available anchors: [${anchorPreview}${moreText}]`);
...
test('HTML quality - W3C HTML validation', async (t) => {
    ...
    // Validate files sequentially to avoid resource issues
    ...
    for (const [filename] of allFiles) {
        try {
            ...
            // Check if file exists - MUST fail if missing (AGENTS.md requirement)
            if (!fs.existsSync(filePath)) {
                // Debug information to understand what's happening
                console.log(`DEBUG: File check failed for ${filename}`);
                console.log(`  - Working directory: ${process.cwd()}`);
                console.log(`  - Expected path: ${filePath}`);
                console.log(`  - Site directory: ${siteDir}`);
                console.log(`  - HTML directory exists: ${fs.existsSync(path.join(siteDir, 'html'))}`);
                console.log(`  - Parent directory exists: ${fs.existsSync(path.dirname(filePath))}`);

                // List files in HTML directory for debugging
                if (fs.existsSync(path.join(siteDir, 'html'))) {
                    const htmlFiles = fs.readdirSync(path.join(siteDir, 'html'));
                    console.log(`  - Files in html directory (first 10): ${htmlFiles.slice(0, 10).join(', ')}`);
                    console.log(`  - free.html in directory: ${htmlFiles.includes('free.html')}`);
                }

                t.fail(
                    `Build artifact missing: ${filename}\n` +
            ...
            if (!result.valid) {
                ...
                result.errors
                    ...
                    .forEach(msg => {
                        ...
                        issues.push(`${filename}${location} - ${msg.message}${extract}`);
                    });
            }
        } catch (error) {
            console.log(`Error validating ${filename}: ${error.message}`);
            issues.push(`${filename} - validation failed: ${error.message}`);
            totalErrors++;
        }
    }
