...
test('build pipeline - compressed files match originals', async (t) => {
    ...
    await fs.writeFile(path.join(t.context.brotliDir, `${testFile}.br`), brotliCompressed);

    // Compress with Gzip
    const gzipCompressed = zlib.gzipSync(Buffer.from(testContent), { level: 9 });
    await fs.writeFile(path.join(t.context.gzipDir, `${testFile}.gz`), gzipCompressed);

    // Verify Brotli decompression
    const brotliPath = path.join(t.context.brotliDir, `${testFile}.br`);
    ...
    const gzipPath = path.join(t.context.gzipDir, `${testFile}.gz`);
...
test('build pipeline - cleans up old artifacts', async (t) => {
    ...
    // Simulate cleanup by removing directories
    await fs.rm(t.context.buildDir, { recursive: true, force: true });
    await fs.rm(t.context.siteDir, { recursive: true, force: true });

    // Recreate directories (simulating clean build)
    await fs.mkdir(t.context.buildDir, { recursive: true });
    await fs.mkdir(t.context.siteDir, { recursive: true });

    // Verify old artifacts are gone
    for (const artifact of oldArtifacts) {
    ...
    const newArtifacts = [
        path.join(t.context.buildDir, 'sitemap.json'),
        path.join(t.context.siteDir, 'index.html'),
    ];

    for (const artifact of newArtifacts) {
        await fs.writeFile(artifact, 'new content');
    }

    // Verify new artifacts exist
    for (const artifact of newArtifacts) {
        t.true(await fileExists(artifact), `${path.basename(artifact)} should exist after rebuild`);
    }
...
test('build pipeline - handles missing required directories', async (t) => {
    ...
    const exists = await fs.access(t.context.buildDir).then(() => true).catch(() => false);
...
`;
        ...
        await fs.writeFile(path.join(t.context.siteDir, 'partial.html'), '<html>Partial</html>');

        // Verify partial artifacts exist
        t.true(await fileExists(path.join(t.context.buildDir, 'sitemap.json')));
        t.true(await fileExists(path.join(t.context.siteDir, 'partial.html')));

        // Simulate recovery by cleaning and rebuilding
        await fs.rm(t.context.buildDir, { recursive: true, force: true });
        await fs.rm(t.context.siteDir, { recursive: true, force: true });
        await fs.mkdir(t.context.buildDir, { recursive: true });
        await fs.mkdir(t.context.siteDir, { recursive: true });
