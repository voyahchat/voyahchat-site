...
test('CSSBuilder.processImageReferences() - should process images with valid mapping', async (t) => {
    ...
    t.true(processed.includes('url(/png/abc123def456.png)'));
...
test('CSSBuilder.saveArtifacts() - should create correct files with hash prefix', async (t) => {
    ...
    // Check that files were created with hash.css naming in css/ directory
    const siteHashedPath = path.join(dir.getSite(), 'css', `${hash}.css`);
    ...
    t.true(siteFileExists, 'Site hashed CSS file should exist with hash.css naming');
    ...
    t.is(artifactInfo.filename, `${hash}.css`);
...
test('CSSBuilder.inlineHashedSVGs() - should inline SVG files as data:uri', async (t) => {
    ...
    // Create test SVG file in svg/ subdirectory
    const testSvg =
        '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 10 10">' +
        '<circle cx="5" cy="5" r="4"/></svg>';
    const svgDir = path.join(dir.getSite(), 'svg');
    await fs.mkdir(svgDir, { recursive: true });
    const hashedSvgPath = path.join(svgDir, 'hashed123.svg');
...
test('CSSBuilder.inlineHashedSVGs() - should use URL encoding, not base64', async (t) => {
    ...
    const svgDir = path.join(dir.getSite(), 'svg');
    await fs.mkdir(svgDir, { recursive: true });
    const hashedSvgPath = path.join(svgDir, 'hashed456.svg');
...
test('CSSBuilder.inlineHashedSVGs() - should handle multiple SVG files', async (t) => {
    ...
    // Create test SVG files in svg/ subdirectory
    const svg1 =
        '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 10 10">' +
    ...
    const svgDir = path.join(dir.getSite(), 'svg');
    await fs.mkdir(svgDir, { recursive: true });
    await fs.writeFile(path.join(svgDir, 'hash1.svg'), svg1);
    await fs.writeFile(path.join(svgDir, 'hash2.svg'), svg2);
...
test('CSSBuilder.saveArtifacts() - should clean up after build failure', async (t) => {
    ...
    fs.writeFile = async (filePath, data) => {
        // Fail only when writing the CSS file to site directory (files with .css extension)
        if (filePath.toString().includes(dir.getSite()) && filePath.toString().includes('.css')) {
    ...
    try {
        ...
        // Verify no partial files remain (files with .css extension)
        const siteFiles = await fs.readdir(dir.getSite()).catch(() => []);
        const hasPartialCSS = siteFiles.some(f => f.endsWith('.css'));
