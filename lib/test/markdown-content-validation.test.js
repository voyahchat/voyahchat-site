...
// =============================================================================
// NEW TESTS: Spacing and list structure validation
// =============================================================================
...
test('Markdown rendering - should preserve space after colon before inline element', (t) => {
    const md = createMarkdownInstance();

    // Test case from changelog: "Дата выпуска: **03.05.2026**"
    const markdown = 'Дата выпуска: **03.05.2026**';
    const html = md.render(markdown);

    // The HTML should contain a space between the colon and the strong tag
    // Expected: "Дата выпуска: <strong>03.05.2026</strong>"
    // NOT: "Дата выпуска:<strong>03.05.2026</strong>"
    t.true(html.includes(': <strong>'), `Expected space after colon, got: ${html}`);
    t.false(html.includes(':<strong>'), 'Should NOT have colon directly before strong tag');
});

test('Markdown rendering - code block inside list item should not break list', (t) => {
    const md = createMarkdownInstance();

    // Test case from install: list with code block inside item 4
    // Note: fenced code block inside list requires empty line before the fence
...
5. Fifth item`;
    ...
    // Should be a single ordered list, not two separate lists
    // Count <ol> opening tags - should be exactly 1
    const olOpenCount = (html.match(/<ol/g) || []).length;
    t.is(olOpenCount, 1, `Expected 1 ordered list, got ${olOpenCount}`);

    // Count closing </ol> tags - should be exactly 1
    const olCloseCount = (html.match(/<\/ol>/g) || []).length;
    t.is(olCloseCount, 1, `Expected 1 closing ordered list tag, got ${olCloseCount}`);

    // All five items should be present
    t.true(html.includes('First item'), 'Should contain "First item"');
    t.true(html.includes('Fifth item'), 'Should contain "Fifth item"');

    // Code block should be inside the list
    t.true(html.includes('adb shell'), 'Should contain code block content');
});

test('Markdown rendering - list items numbered correctly when code is inside item', (t) => {
    const md = createMarkdownInstance();
...
5. Five`;
    ...
    // Should NOT have start attribute (all items in one list)
    t.false(html.includes('start='), `Should NOT have start attribute, got: ${html}`);

    // All items should be in single list
    const olOpenCount = (html.match(/<ol/g) || []).length;
    t.is(olOpenCount, 1, 'Expected 1 ordered list');
});
