...
class HTMLBuilder {
    ...
    static attrClass(value) {
        return /\s/.test(value) ? `class="${value}"` : `class=${value}`;
    }
    ...
    /**
     * Extract local resource file paths from HTML for dependency tracking
     * @param {string} html - The rendered HTML
     * @param {string} siteDir - Absolute path to site directory
     * @returns {string[]} Array of absolute paths to local resources
     */
    static extractDependencies(html, siteDir) {
        const fsSync = require('fs');
        const paths = [];
        const urlRegex = /(?:src|href|content)=["'](\/[^"']+)["']/gi;
        let match;
        while ((match = urlRegex.exec(html)) !== null) {
            ...
            const filePath = path.join(siteDir, url.substring(1));
    ...
    async build() {
        ...
        const [
            ...
            // Load video hash mapping
            fs.readFile(path.join(BUILD_DIR, 'hash-video.json'), 'utf-8')
                .then(JSON.parse)
                .catch(() => ({})),
        ]);
        ...
        const videoLovtoUrl = videoHash['lovto.mp4']
            ? `/${videoHash['lovto.mp4']}`
            : '';
        ...
        const renderPromises = Object.keys(sitemap.pages).map(async (pageUrl) => {
            ...
            pageData.videoLovto = videoLovtoUrl;

            const context = {
                sitemap,
                hash: hashData,
                helpers: { cleanUrl: HTMLBuilder.cleanUrl, attrClass: HTMLBuilder.attrClass },
                page: pageData,
                content: contentHtml,
            };
        ...
        const writePromises = htmlPages.map(async (page) => {
            ...
            if (!this.options.skipWrite) {
                ...
                try {
                    ...
                    // HTML content depends on all local resources referenced inside it.
                    // Their mtimes are already set from git timestamps of their sources.
                    // Take the max of current HTML mtime and all dependency mtimes.
                    const depPaths = HTMLBuilder.extractDependencies(page.finalProcessedHtml, SITE_DIR);
                    if (depPaths.length > 0) {
                        const currentMtime = (await fs.stat(page.outputPath)).mtime.getTime();
                        let maxMtime = currentMtime;
                        for (const depPath of depPaths) {
                            try {
                                const depMtime = (await fs.stat(depPath)).mtime.getTime();
                                if (depMtime > maxMtime) {
                                    maxMtime = depMtime;
                                }
                            } catch { /* skip missing deps */ }
                        }
                        if (maxMtime !== currentMtime) {
                            const date = new Date(maxMtime);
                            await fs.utimes(page.outputPath, date, date);
                        }
                    }
...
module.exports = {
    ...
    extractDependencies: HTMLBuilder.extractDependencies,
