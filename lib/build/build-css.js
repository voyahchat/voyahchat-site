...
class CSSBuilder {
    ...
    static getSourcePaths(dir = Dir) {
        ...
        return levels.map((level) => path.resolve(projectRoot, level));
    }
    ...
    async saveArtifacts(css, hash, sourceFiles) {
        ...
        allHashes[this.bundle] = {
            ...
            url: `/${hash}.css`,
            sourceFiles: (sourceFiles || []).map((file) => path.relative(this.dir.getRoot(), file)),
        };
        ...
        // Set timestamp from source CSS files (use latest) - CRITICAL: build must fail if timestamp cannot be set
        if (sourceFiles && sourceFiles.length > 0) {
            try {
                await Timestamp.setTimestamp(siteHashedPath, sourceFiles);
            } catch (timestampError) {
                const errorMsg = 'CRITICAL: Failed to set timestamp for '
                    + `${hashedFilename} from source CSS files: ${timestampError.message}`;
