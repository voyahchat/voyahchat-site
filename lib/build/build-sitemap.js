 ...
 * @param {Object} dir - Directory instance (TestDir or Dir)
 * @returns {string} Rendered HTML
 */
function processDocument(url, filePath, sitemap, _options = {}, dir = Dir) {
...
class SitemapBuilder {
    ...
    /**
     * Detect whether a sitemap entry is a cross-section link: an absolute URL
     * that points outside its parent section. Such entries reuse the canonical
     * page record and may carry their own menu label via meta.navLabel.
     * @param {string} fullUrl - Resolved absolute URL of the entry
     * @param {string} parentUrl - Parent URL (empty at top level)
     * @returns {boolean} True if the entry links across sections
     */
    static isCrossSection(fullUrl, parentUrl) {
        if (!parentUrl) {
            return false;
        }
        const cleanParent = parentUrl.replace(/\/$/, '');

        return !fullUrl.startsWith(`${cleanParent}/`);
    }
     ...
     */
    static processSitemap(items, parentUrl = '', dir = Dir) {
        const sitemap = [];
        const pages = {};
        ...
        function processItems(navItems, parentUrlValue, sitemapArray) {
            navItems.forEach((item) => {
                ...
                // Simple string item - parse it
                    ...
                    // Cross-section link: absolute URL pointing outside its parent.
                    // It reuses the canonical page record (do not overwrite name/title)
                    // and may carry its own menu label via meta.navLabel.
                    const crossSection = SitemapBuilder.isCrossSection(fullUrl, parentUrlValue);

                    // Add to sitemap array (carry navLabel on the tree node if present)
                    if (crossSection && parsed.meta.navLabel) {
                        sitemapArray.push({ [fullUrl]: parsed.meta.navLabel });
                    } else {
                        sitemapArray.push(fullUrl);
                    }

                    // Add to pages object, but never clobber the canonical record of a
                    // cross-section link (it was defined in its home section).
                    if (crossSection && pages[fullUrl]) {
                        return;
                    }
        ...
        // Load password data if available
        let passwordData = null;
        try {
            const passwordJsonPath = path.join(
                dir.getRoot(),
                'external',
                'voyahchat-content',
                'common',
            ...
            if (fsSync.existsSync(passwordJsonPath)) {
                const passwordContent = fsSync.readFileSync(passwordJsonPath, 'utf8');
                passwordData = JSON.parse(passwordContent);
            }
        } catch (error) {
            // Password file doesn't exist or is invalid, continue without it
            if (process.env.NODE_ENV !== 'test') {
                console.warn('Warning: Could not load password.json:', error.message);
            }
        }
        ...
        Object.values(pages).forEach((pageData) => {
            ...
            // Special handling for password page - prepend dynamic password to title
            if (pageData.url === '/common/password' && passwordData && passwordData.password) {
                titleParts.unshift(passwordData.password);
            }
     ...
     */
    static async getLastModFromGit(filePaths, repoPath, _dir = Dir) {
    ...
    async build() {
        try {
            ...
            await this.dir.ensure(buildDir);

            // Process sitemap items into structure
            const result = SitemapBuilder.processSitemap(navigation, '', this.dir);
            ...
            Object.entries(result.pages).forEach(([url, pageData]) => {
                ...
                if (filePath && url) {
                    // Map full file path to URL (last URL wins for backward compatibility)
                    md2url[filePath] = url;

                    // Map full file path to all URLs (for shared files across multiple paths)
                    if (!md2urls[filePath]) {
                        md2urls[filePath] = [];
                    }
                    md2urls[filePath].push(url);
            ...
            if (!this.options.skipWrite) {
                ...
                const lastModDates = await SitemapBuilder.getLastModFromGit(filePaths, contentDir, this.dir);
                ...
                // Set timestamp from all markdown files (use latest)
                const markdownFiles = Object.values(pages).map(pageData =>
                    path.join(contentDir, pageData.file)).filter(Boolean);

                if (markdownFiles.length > 0) {
                    await Timestamp.setTimestamp(sitemapXmlPath, markdownFiles);
                }

                // Collect statistics for sitemap.xml
                const statsInstance = new Stats('build-sitemap.json', this.dir.getBuild());
