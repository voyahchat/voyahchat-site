...
// Test: Hash-prefixed JS files (_j{hash})
test('JSBuilder.saveArtifacts() - should create JS files with _j hash prefix', async (t) => {
    const testDir = new TestDir();
    ...
    // Check that filename has _j prefix
    t.true(result.filename.startsWith('_j'), 'Filename should start with _j prefix');
    t.is(result.filename.length, 18, 'Filename should be _j + 16 char hash');

    // Check that file was created
    const siteHashedPath = path.join(testDir.getSite(), result.filename);
    const fileExists = await fs.access(siteHashedPath)
        .then(() => true)
        .catch(() => false);

    t.true(fileExists, 'JS file should exist with _j prefix');
    ...
    t.true(hashData['test-bundle'].filename.startsWith('_j'));
});
