...
const { loadExternalRepos, loadBemDeclaration, getTemplateFilesForBlocks } = require('./utils');
...
class Timestamp {
    ...
    /**
     * Get the latest timestamp from templates used by a specific page layout
     * @param {string} layout - The layout template path (e.g., 'blocks/page/page.njk')
     * @returns {Promise<number|null>} Latest Unix timestamp or null
     */
    static async getLayoutTemplateTimestamp(layout) {
        const instance = new Timestamp();
        return instance._getLayoutTemplateTimestamp(layout);
    }
    ...
    async _getSourceTimestamp(sourcePath) {
        ...
        if (Array.isArray(sourcePath)) {
            // Clear cache for these files to ensure fresh timestamps
            for (const path of sourcePath) {
                const cacheKey = `${path}:auto`;
    ...
    async _getFileTimestamps(filePaths, repoPath = null) {
        ...
        for (const filePath of filePaths) {
            ...
            if (this.cache.has(cacheKey)) {
                ...
                if (!filesByRepo.has(fileRepo)) {
                    filesByRepo.set(fileRepo, []);
                }
                filesByRepo.get(fileRepo).push({
                    filePath,
                    relativePath: path.relative(fileRepo, filePath),
                    cacheKey,
                });
            }
        }

        // Process files in each repository
        for (const [repo, repoFiles] of filesByRepo.entries()) {
            if (repoFiles.length === 1) {
                // Single file, process individually
                const fileData = repoFiles[0];
                const timestamp = await this._getFileTimestamp(fileData.filePath, repo);
                ...
                try {
                    ...
                    for (const line of lines) {
                        if (line.match(/^\d+$/)) {
                            ...
                            // Only set the timestamp if we haven't seen this file before
                            // or if the current timestamp is newer
                            const existingTimestamp = dateMap.get(line);
                            if (
                                existingTimestamp === null ||
                                existingTimestamp === undefined ||
                                currentDate > existingTimestamp
                            ) {
    ...
    async _getLatestTimestamp(filePaths, repoPath = null) {
        ...
        for (const [, timestamp] of timestamps.entries()) {
    ...
    async _getLayoutTemplateTimestamp(layout) {
        // Check cache first
        const cacheKey = `layout_template_timestamp:${layout}`;
        if (this.cache.has(cacheKey)) {
            return this.cache.get(cacheKey);
        }

        // Determine bundle from layout
        let bundle = 'page'; // default
        if (layout.includes('page-index')) {
            bundle = 'page-index';
        }

        // Load the BEM declaration to get the list of blocks
        const blocks = loadBemDeclaration(bundle, Dir);

        // Get template files for these blocks
        const templateFiles = getTemplateFilesForBlocks(blocks, Dir);

        // Get the latest timestamp from all template files
        const latestTimestamp = await this._getLatestTimestamp(templateFiles);

        // Cache the result
        this.cache.set(cacheKey, latestTimestamp);
        return latestTimestamp;
    }
