...
test('AssetsBuilder.main() - should trigger rebuild when assets missing', async (t) => {
    ...
    // Mock console.error to check for missing assets message
    let errorOutput = '';
    ...
    // Should have logged missing assets error
    t.true(errorOutput.includes('Missing assets'));

    // Note: The actual behavior might vary based on the copyAssets implementation
    // This test assumes that missing assets in the JSON don't prevent processing of new ones
});
...
test('Integration - robots.txt is excluded from compression', async (t) => {
    ...
    const originalSiteDir = process.env.SITE_DIR;
    process.env.SITE_DIR = testSiteDir;
    ...
    try {
        ...
        // Restore environment
        if (originalSiteDir) {
            process.env.SITE_DIR = originalSiteDir;
        } else {
