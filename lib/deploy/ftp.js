...
class FTP {
    ...
    /**
     * Download timestamps-server.json from server (cache)
     */
    async downloadServerTimestampsFile() {
        try {
            const siteDir = this.dir.getSite();
            const serverTimestampsPath = path.join(siteDir, 'timestamps-server.json');
            await this.client.downloadTo(serverTimestampsPath, 'timestamps-server.json');
            const parsed = JSON.parse(fs.readFileSync(serverTimestampsPath, 'utf8'));
            // Also save locally for syncServerTimestamps to use
            return parsed;
        } catch (err) {
            return {};
        }
    ...
    /**
     * Upload timestamps-server.json to server (cache)
     */
    async uploadServerTimestampsFile() {
        try {
            const siteDir = this.dir.getSite();
            const serverTimestampsPath = path.join(siteDir, 'timestamps-server.json');
            await this.client.uploadFrom(serverTimestampsPath, 'timestamps-server.json');
            return true;
        } catch (err) {
            console.error('Failed to upload timestamps-server.json:', err.message);
            return false;
        }
    }
    ...
    /**
     * List all files on FTP server recursively
     */
    async listRemoteFiles(remotePath = '.') {
        const files = [];

        const listDir = async (currentPath) => {
            ...
            for (const item of list) {
                ...
                if (item.type === 2 || item.type === 'd' || item.isDirectory) {
    ...
    /**
     * Scan FTP server and create/update timestamps-server.json
     * Returns the server timestamps object
     */
    async syncServerTimestamps(options = {}) {
        const progress = options.progress !== undefined
            ? options.progress
            : (this.config && this.config.progress !== false);
        const siteDir = this.dir.getSite();
        const serverTimestampsPath = path.join(siteDir, 'timestamps-server.json');

        if (progress) {
            console.log('Scanning files on FTP server...');
        }
        const remoteFiles = await this.listRemoteFiles('.');

        if (progress) {
            console.log(`Found ${remoteFiles.length} files on server`);
        }
        ...
        for (const file of remoteFiles) {
            ...
            if (showProgress && (processed % 50 === 0 || isZip)) {
                process.stdout.write(`Getting timestamps: ${processed}/${remoteFiles.length}\r`);
            }

            const mdtmTimestamp = await this.getRemoteTimestamp(file.path);
            if (mdtmTimestamp) {
                timestamps[file.path] = mdtmTimestamp.getTime();
                mdtmUsed++;
            } else if (file.modified) {
                timestamps[file.path] = file.modified.getTime();
                listUsed++;
            } else {
                failed++;
                if (showProgress && isZip) {
                    console.log(`Warning: no timestamp for ${file.path}`);
                }
            }
        }
        ...
        // Write using common function (excludes buildTime, sorts keys)
        TimestampsJson.writeTimestamps(timestamps, serverTimestampsPath, true);

        if (progress) {
            console.log(`Updated timestamps-server.json with ${Object.keys(timestamps).length} entries`);
            const zipCount = Object.keys(timestamps).filter(k => k.endsWith('.zip')).length;
            console.log(`  ZIP files: ${zipCount}`);
            console.log(`  Timestamps via MDTM: ${mdtmUsed}, via LIST: ${listUsed}, failed: ${failed}`);
        }
    ...
    /**
     * Get list of files that exist on server but not locally
     */
    getOrphanedFiles(localTimestamps, serverTimestamps) {
        const protectedFiles = new Set([
            'timestamps.json',
            'timestamps-server.json',
        ]);

        const localFiles = new Set(Object.keys(localTimestamps));
        return Object.keys(serverTimestamps)
            .filter(f => !localFiles.has(f) && !protectedFiles.has(f))
            .sort();
    }

    /**
     * Remove orphaned files from FTP server
     */
    async cleanOrphanedFiles(localTimestamps, serverTimestamps, progress = true) {
        const orphanedFiles = this.getOrphanedFiles(localTimestamps, serverTimestamps);
        ...
        if (progress) {
            console.log(`\nCleaning ${orphanedFiles.length} orphaned files...`);
        ...
        for (const file of orphanedFiles) {
            try {
                ...
                deleted++;
                if (progress) {
                    process.stdout.write(`Deleted: ${file}\r`);
                }
            } catch (err) {
                failed++;
                if (progress) {
    ...
    async deploy(options = {}) {
        ...
        try {
            ...
            if (!fs.existsSync(path.join(siteDir, this.timestampsFile))) {
                ...
                localTimestamps = new TimestampsBuilder(this.dir).generate();
            } else {
            ...
            if (!dryRun && !force) {
                // Try to download existing timestamps-server.json from server (cache)
                serverTimestamps = await this.downloadServerTimestampsFile();
                let usedCache = false;

                if (Object.keys(serverTimestamps).length > 0) {
                    // Cache exists - verify it's still valid by checking file count via LIST
                    const remoteFiles = await this.listRemoteFiles('.');
                    if (remoteFiles.length === Object.keys(serverTimestamps).length) {
                        // Cache matches current server state
                        usedCache = true;
                        if (progress) {
                            console.log(`Using cached server timestamps (${remoteFiles.length} files)`);
                        }
                    } else {
                        if (progress) {
                            console.log(
                                `Cache outdated (cache: ${Object.keys(serverTimestamps).length}, ` +
                                `server: ${remoteFiles.length}), rescanning...`,
                            );
                        }
                    }
                }
                ...
                if (!usedCache) {
                    // No cache or outdated, scan server
                    if (progress) {
                        console.log('Scanning server files...');
                    }
                    serverTimestamps = await this.syncServerTimestamps({ progress });
                    // Upload cache for next time
                    if (progress) {
                        console.log('Uploading timestamps cache...');
                    }
                    await this.uploadServerTimestampsFile();
                }
                ...
                if (Object.keys(serverTimestamps).length === 0) {
                    ...
                    if (progress) {
                        ...
                        if (toUpload > 0) {
                            console.log(`${toUpload} files need to be uploaded`);
                        } else {
            ...
            for (const file of filesToUpload) {
                if (dryRun) {
                    ...
                    await this.processFile(
                        ...
                        file.reason,
                        file.localTimestamp,
                        file.serverTimestamp,
                        options.verbose,
                    );
                }
            }

            // Clean orphaned files from server
            if (!dryRun && !force && serverTimestamps && Object.keys(serverTimestamps).length > 0) {
                await this.cleanOrphanedFiles(localTimestamps, serverTimestamps, progress);
            }
