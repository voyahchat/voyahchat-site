...
test('Whitespace removal - should remove whitespace between block elements', (t) => {
    ...
    // Note: Space after heading tag is expected for anchor links
    t.false(/(?!<h[1-6][^>]*>)>\s+<(?!\/?a)/.test(html));
...
`;
    ...
    // Note: Space after heading tag is expected for anchor links
    t.false(/(?!<h[1-6][^>]*>)>\s+<(?!\/?a)/.test(html), 'Should not have whitespace between block elements');
});
...
`;
    ...
    // Verify whitespace removal (excluding heading anchor links)
    t.false(/(?!<h[1-6][^>]*>)>\s+<(?!\/?a)/.test(html));
});
