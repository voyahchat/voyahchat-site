...
class CompressionBuilder {
    ...
    async compressFile(file, stats) {
        ...
        // Normalize OS byte (byte 9) to 0xFF (unknown) for cross-platform determinism.
        // Node.js zlib sets this byte based on the build platform: 0x03 on Linux,
        // 0x13 on macOS. This causes identical source content to produce different
        // gzip files across platforms, breaking manifest hash comparison and triggering
        // unnecessary re-uploads when deploying from different OS.
        gzipContent[9] = 0xFF;
        ...
        if (!this.options.skipWrite) {
            ...
            // Set timestamps from original uncompressed file
            const originalPath = path.join(this.siteDir, file);
            await Promise.all([
                Timestamp.setTimestamp(brotliPath, originalPath),
                Timestamp.setTimestamp(gzipPath, originalPath),
                ...(zstdBetterThanBrotli ? [Timestamp.setTimestamp(zstdPath, originalPath)] : []),
            ]);
        }
