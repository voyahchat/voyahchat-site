...
test('processSitemap() - should carry navLabel for cross-section links without clobbering the canonical page', (t) => {
    // Arrange: /help references /partners/goodbro (a cross-section link) with its
    // own menu label, while /partners defines the canonical Goodbro page.
    const input = [
        'Home [/, index.md]',
        {
            'Help [/help, help/index.md]': [
                'Master [ma, help/ma.md]',
                'Link [/partners/goodbro, partners/goodbro.md, { navLabel: \'Страхование\' }]',
            ],
        },
        {
            'Partners [/partners, partners/index.md]': [
                'Goodbro — x [goodbro, partners/goodbro.md]',
            ],
        },
    ];
    ...
    // Assert: tree node under /help is a labeled-leaf object (not a plain string)
    const helpChildren = result.sitemap[1]['/help'];
    t.deepEqual(helpChildren[1], { '/partners/goodbro': 'Страхование' });

    // Assert: tree node under /partners stays the plain string
    const partnersChildren = result.sitemap[2]['/partners'];
    t.is(partnersChildren[0], '/partners/goodbro');

    // Assert: canonical page record is NOT overwritten by the /help line
    t.is(result.pages['/partners/goodbro'].name, 'Goodbro — x');

    // Assert: exactly one page record for the shared URL (no duplicate)
    t.is(Object.keys(result.pages).filter((u) => u === '/partners/goodbro').length, 1);
});
