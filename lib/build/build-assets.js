...
class AssetsBuilder {
    ...
    /**
     * Validate that all assets from repositories are included in the mapping
     * @param {Object} assetsJson - Current assets mapping
     * @returns {Promise<boolean>} True if mapping is complete, false otherwise
     */
    async validateMappingCompleteness(assetsJson) {
        const expectedAssets = new Set();

        // Collect all PDF and ZIP files from repositories
        for (const repo of this.SOURCE_REPOS) {
            try {
                await fs.access(repo.path);
                const files = await fs.readdir(repo.path);
                const assetFiles = files.filter((file) =>
                    file.endsWith('.pdf') || file.endsWith('.zip'));

                for (const file of assetFiles) {
                    const fullSourceUrl = `${repo.baseUrl}/${file}`;
                    expectedAssets.add(fullSourceUrl);
                }
            } catch (error) {
                console.warn(`Warning: Cannot validate mapping for ${repo.name}: ${error.message}`);
                // Skip this repo if it doesn't exist
            }
        }

        // Check if all expected assets are in the mapping
        const mappedAssets = new Set(Object.keys(assetsJson));

        for (const expectedUrl of expectedAssets) {
            if (!mappedAssets.has(expectedUrl)) {
                return false; // Missing asset found
            }
        }

        return true; // All assets are mapped
    }
    ...
    async copyAssets() {
        ...
        for (const repo of this.SOURCE_REPOS) {
            try {
                ...
                console.warn(`Warning: Source directory not found: ${repo.path}, skipping...`);
            ...
            try {
                ...
                for (const file of assetFiles) {
                    ...
                    try {
                        ...
                        throw new Error(
                            `CRITICAL: Failed to set timestamp for ${file} from ${sourcePath}: `
                ...
                // Continue with other repos even if this one fails
            }
        }
     ...
     * @returns {Promise<void>}
     */
    async copyAssetsToSite(assetsJson, stats) {
        await this.dir.ensure(this.siteDir);
        ...
        for (const [sourceUrl, localPath] of Object.entries(assetsJson)) {
            ...
            try {
                ...
                console.warn(`Warning: Asset file not found: ${sourcePath}, skipping...`);
            ...
            if (!this.options.skipWrite) {
                ...
                // Set timestamp from original source file in external repo
                // CRITICAL: build must fail if timestamp cannot be set
                const timestampSource = originalSourcePath || sourcePath;
                try {
                    await Timestamp.setTimestamp(destPath, timestampSource);
                } catch (timestampError) {
                    const errorMsg = 'CRITICAL: Failed to set timestamp for '
                        + `${fileName} from ${timestampSource}: ${timestampError.message}`;
     ...
     */
    async build() {
        await this.dir.ensure(this.ASSETS_DIR);
        ...
        try {
            ...
            // Check 1: Do all mapped assets exist?
            const missingAssets = await this.validateAssetsExist(assetsJson);
            if (missingAssets.length > 0) {
                needsRebuild = true;
            }

            // Check 2: Are all repository assets mapped?
            if (!needsRebuild) {
                const isComplete = await this.validateMappingCompleteness(assetsJson);
                if (!isComplete) {
                    needsRebuild = true;
                }
            }
        ...
        const stats = new Stats('build-assets.json', this.dir.getBuild());
        stats.stats = existingStats; // Load existing data

        // Copy assets from external repos
        await this.copyAssetsToSite(assetsJson, stats);
