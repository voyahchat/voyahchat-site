...
class HTMLBuilder {
    ...
    async build() {
        ...
        const BUILD_DIR = this.options.buildDir || Dir.getBuild();
        const SITE_DIR = this.options.siteDir || Dir.getSite();
        ...
        await Dir.ensure(BUILD_DIR);
        if (!this.options.skipWrite) {
            await Dir.ensure(SITE_DIR);
        }
        ...
        const renderPromises = Object.keys(sitemap.pages).map(async (pageUrl) => {
            ...
            // Create flat file structure: all HTML files in site root
            const flatFilename = HTMLBuilder.urlToFlatFilename(pageData.url);
            const outputPath = path.join(SITE_DIR, flatFilename);
