...
function generateNginxConfig(testDir) {
    const env = nunjucks.configure(path.join(Dir.getRoot(), 'config'), {
        autoescape: false,
        trimBlocks: true,
        lstripBlocks: true,
    });

    const templatePath = path.join(Dir.getRoot(), 'config', 'config-nginx.njk');
...
test('nginx config() - handles non-compressible files', async (t) => {
    ...
    t.true(config.includes('location ~* \\.(jpg|jpeg|png|gif|ico|pdf|zip|ini|mp4)$'));
...
test('nginx config() - has cache control mapping for static files', async (t) => {
    ...
    t.true(config.includes('~*\\.(css|js|svg|jpg|jpeg|png|gif|ico|ini|mp4)$'));
});
