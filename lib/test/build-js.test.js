...
test('JSBuilder.generateBundle() - should bundle multiple JS files in correct order', async (t) => {
    ...
    await fs.writeFile(path.join(block1Dir, 'block1.js'), 'window.block1 = true;');
    await fs.writeFile(path.join(block2Dir, 'block2.js'), 'window.block2 = true;');
...
test('JSBuilder.minifyJS() - should minify JavaScript with Terser', async (t) => {
    ...
    t.true(minified.includes('function') || minified.includes('test'));
});
...
// Test: Hash-based JS files ({hash}.js)
test('JSBuilder.saveArtifacts() - should create JS files with hash.js naming', async (t) => {
    const testDir = new TestDir();
    const builder = new JSBuilder('test-bundle', testDir);

    const js = 'const x = 1; console.log(x);';
    const sourceFiles = [];

    const result = await builder.saveArtifacts(js, sourceFiles);

    // Check that filename has .js extension
    t.true(result.filename.endsWith('.js'), 'Filename should end with .js extension');
    t.is(result.filename.length, 19, 'Filename should be 16 char hash + .js');

    // Check that file was created in js/ directory
    const siteHashedPath = path.join(testDir.getSite(), 'js', result.filename);
    const fileExists = await fs.access(siteHashedPath)
        .then(() => true)
        .catch(() => false);

    t.true(fileExists, 'JS file should exist with hash.js naming');

    // Check hash info
    const hashPath = path.join(testDir.getBuild(), 'hash-js.json');
    const hashData = JSON.parse(await fs.readFile(hashPath, 'utf8'));

    t.is(hashData['test-bundle'].filename, result.filename);
    t.is(hashData['test-bundle'].url, `/${result.filename}`);
    t.true(hashData['test-bundle'].filename.endsWith('.js'));
});
