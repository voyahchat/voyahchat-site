...
const { loadBemDeclaration, getTemplateFilesForBlocks } = require('../build/utils');
...
// Tests for BEM declaration and template functions

test('loadBemDeclaration() - should load BEM declaration file', async (t) => {
    const testDir = new TestDir();

    // Create test BEM declaration
    const blocksDir = path.join(testDir.getRoot(), 'blocks');
    await fs.mkdir(blocksDir, { recursive: true });
    const pageDir = path.join(blocksDir, 'test-page');
    await fs.mkdir(pageDir, { recursive: true });
    const bemdeclPath = path.join(pageDir, 'test-page.bemdecl.js');
    await fs.writeFile(bemdeclPath, 'module.exports = ["block1", "block2"];');

    // Load BEM declaration
    const blocks = loadBemDeclaration('test-page', testDir);

    t.deepEqual(blocks, ['block1', 'block2']);
});

test('loadBemDeclaration() - should return empty array when file does not exist', (t) => {
    const testDir = new TestDir();

    // Try to load non-existent BEM declaration
    const blocks = loadBemDeclaration('non-existent', testDir);

    t.deepEqual(blocks, []);
});

test('getTemplateFilesForBlocks() - should find template files for blocks', async (t) => {
    const testDir = new TestDir();

    // Create test template files
    const blocksDir = path.join(testDir.getRoot(), 'blocks');
    await fs.mkdir(path.join(blocksDir, 'block1'), { recursive: true });
    await fs.writeFile(path.join(blocksDir, 'block1', 'block1.njk'), 'template1');
    await fs.mkdir(path.join(blocksDir, 'block2'), { recursive: true });
    await fs.writeFile(path.join(blocksDir, 'block2', 'block2.njk'), 'template2');

    // Get template files
    const templateFiles = getTemplateFilesForBlocks(['block1', 'block2'], testDir);

    t.is(templateFiles.length, 2);
    t.true(templateFiles.some(f => f.includes('block1.njk')));
    t.true(templateFiles.some(f => f.includes('block2.njk')));
});

test('getTemplateFilesForBlocks() - should handle missing template files', async (t) => {
    const testDir = new TestDir();
    ...
    // Get template files for both blocks
    const templateFiles = getTemplateFilesForBlocks(['block1', 'block2'], testDir);

    t.is(templateFiles.length, 1);
    t.true(templateFiles.some(f => f.includes('block1.njk')));
});

test('getTemplateFilesForBlocks() - should find external template files', async (t) => {
    const testDir = new TestDir();

    // Create external template file
    const externalBlocksDir = path.join(testDir.getRoot(), 'external', 'adaptive-layout', 'blocks');
    await fs.mkdir(path.join(externalBlocksDir, 'external-block'), { recursive: true });
    await fs.writeFile(path.join(externalBlocksDir, 'external-block', 'external-block.njk'), 'external template');

    // Get template files
    const templateFiles = getTemplateFilesForBlocks(['external-block'], testDir);

    t.is(templateFiles.length, 1);
    t.true(templateFiles.some(f => f.includes('external-block.njk')));
});
