...
class FTP {
    ...
    async deploy(options = {}) {
        ...
        try {
            ...
            // Get actual server file sizes via LIST (ground truth)
            let serverFiles = {};
            if (!dryRun) {
                if (progress) {
            ...
            if (!force && !dryRun && !noServerManifest) {
            ...
            if (force || dryRun || noServerManifest) {
                ...
                // Safety limit: abort if too many files would be deleted
                const serverFileCount = Object.keys(serverManifest).length;
