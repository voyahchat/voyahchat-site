...
class HTMLBuilder {
    ...
    async build() {
        ...
        const renderPromises = Object.keys(sitemap.pages).map(async (pageUrl) => {
            ...
            // Get timestamp from source markdown file (but not for root page)
            let lastUpdated = null;
            if (pageData.file && pageUrl !== '/') {
        ...
        // But preserve spaces between inline elements like </code><a> to prevent text concatenation
        const transformedPages = validPages.map(page => ({
            ...page,
            transformedHtml: page.finalHtml
                // First, protect spaces between inline elements that should have spacing
                .replace(
                    /<\/(code|span|strong|em|b|i|u|small|sub|sup)>\s+<(a|code|span|strong|em|b|i|u|small|sub|sup)/g,
                    (match) => match.replace(/\s+/g, ' '),
                )
                // Remove newlines and spaces between most tags
                .replace(/>\s+</g, '><')
                // Restore protected spaces between inline elements
                .replace(
                    /<\/(code|span|strong|em|b|i|u|small|sub|sup)><(a|code|span|strong|em|b|i|u|small|sub|sup)/g,
                    '</$1> <$2',
                )
