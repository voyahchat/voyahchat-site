    ...
    async deploy(options = {}) {
        ...
        try {
            ...
            if (!dryRun) {
                ...
                // Verify via recursive LIST (one operation for all files)
                if (progress) {
                    console.log('Verifying files on server...');
                }
                const serverFiles = await this.listServerFiles(progress);
                let missing = 0;
                let sizeMismatch = 0;
                for (const [filePath, info] of Object.entries(localManifest)) {
                    if (!(filePath in serverFiles)) {
                        missing++;
                        if (progress) {
                            console.error(`VERIFY FAILED: ${filePath} not found on server`);
                        }
                    } else if (serverFiles[filePath] !== info.size) {
                        sizeMismatch++;
                        if (progress) {
                            console.error(
                                `VERIFY FAILED: ${filePath} size mismatch ` +
                                `(expected ${info.size}, got ${serverFiles[filePath]})`,
                            );
                        }
                    } else if (progress) {
                ...
                const ok = localFileCount - missing - sizeMismatch;
                if (progress) {
                    console.log(`  Verified: ${ok}, Missing: ${missing}, Size mismatch: ${sizeMismatch}`);
                }
                if (missing > 0 || sizeMismatch > 0) {
                    this.stats.errors += missing + sizeMismatch;
                    throw new Error(
                        `Verification failed: ${missing} missing, ${sizeMismatch} size mismatch`,
                    );
                }
                if (progress) {
