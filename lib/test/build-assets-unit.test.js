...
test('AssetsBuilder() - should merge statistics correctly', async (t) => {
    ...
    const testBuildDir = t.context.dir.getBuild();
    ...
    // Act - use TestDir instead of overriding Dir
    // Load existing stats
    const loadedStats = await Stats.loadFromFile('build-assets.json', t.context.dir);

    // Create new stats instance and merge
    const stats = new Stats('build-assets.json', t.context.dir);
...
test('AssetsBuilder.build() - should include .htaccess in config files count', async (t) => {
    const testRoot = t.context.dir.getRoot();
    const configDir = t.context.dir.getConfig();
    const siteDir = t.context.dir.getSite();
    ...
    // Remove any existing build-assets.json to ensure clean test
    try {
        await fs.unlink(path.join(buildDir, 'build-assets.json'));
    } catch (error) {
        // File doesn't exist, that's fine
    }

    const builder = new AssetsBuilder({ skipWrite: false, dir: t.context.dir });
    builder.projectRoot = testRoot;
    builder.CONFIG_DIR = configDir;
    builder.siteDir = siteDir;
    builder.ASSETS_DIR = path.join(testRoot, '.assets');
    builder.SOURCE_REPOS = []; // No external repos for this test

    const result = await builder.build();

    t.is(result.configFilesProcessed, 2); // robots.txt + .htaccess
    t.true(result.assetsProcessed >= 0, 'Should process non-negative number of assets');
    t.true(result.totalFiles >= 2, 'Should have at least 2 total files (config files)');
});

test('AssetsBuilder.build() - should copy .ini files from voyahchat-content repository', async (t) => {
    const testRoot = t.context.dir.getRoot();
    const externalDir = path.join(t.context.dir.getRoot(), 'external');
...
test('AssetsBuilder.build() - records missing repos in meta file', async (t) => {
    const testRoot = t.context.dir.getRoot();
    const siteDir = t.context.dir.getSite();
    const buildDir = t.context.dir.getBuild();

    const builder = new AssetsBuilder({ skipWrite: false }, t.context.dir);
    builder.projectRoot = testRoot;
    builder.siteDir = siteDir;
    builder.SOURCE_REPOS = [
        { name: 'voyahchat-install', path: '/nonexistent/install' },
        { name: 'voyahchat-content', path: '/nonexistent/content' },
    ];
    ...
    t.true(metaExists, 'Meta file should be created');

    if (metaExists) {
        const meta = JSON.parse(await fs.readFile(metaPath, 'utf8'));
        t.deepEqual(meta.missingRepos, ['voyahchat-install', 'voyahchat-content']);
        t.deepEqual(meta.availableRepos, []);
    }
});
