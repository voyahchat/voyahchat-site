...
function transformAnchorLink(originalHref, env) {
    ...
    // But first, try to use the current heading stack context if available
    if (env && env._headingStack && env._headingStack.length > 0) {
        ...
        // Try to find if this anchor corresponds to a heading that would be at the next level
        // For example, if we have "Yandex" in stack and looking for "windows",
        // it would likely be "Yandex -> Windows" (h2 under h1)
        ...
        // Only return if it's different from the original (indicates successful transformation)
