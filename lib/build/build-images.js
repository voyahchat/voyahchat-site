        ...
        try {
            ...
            if (!isDuplicate) {
                ...
                if (!this.skipWrite) {
                    ...
                    // Set timestamp from source image - CRITICAL: build must fail if timestamp cannot be set
                    try {
                        await Timestamp.setTimestamp(outputPath, imagePath);
                    } catch (timestampError) {
                        const errorMsg = 'CRITICAL: Failed to set timestamp for '
                            + `${hashedFilename} from ${imagePath}: ${timestampError.message}`;
    ...
    async checkModernFormat(imagePath, hash, format) {
        try {
            ...
            try {
                ...
                // Set timestamp from source format file - CRITICAL: build must fail if timestamp cannot be set
                try {
                    await Timestamp.setTimestamp(outputPath, formatPath);
                } catch (timestampError) {
                    const errorMsg = 'CRITICAL: Failed to set timestamp for '
                        + `${hash}.${format} from ${formatPath}: ${timestampError.message}`;
                    throw new Error(errorMsg);
                }
