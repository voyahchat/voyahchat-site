...
Mac instructions for One
    ...
    // Use a mock file path that doesn't exist to make the test completely independent
    const env = { page: { inputPath: './external/voyahchat-content/test/test-file.md' } };
    const html = md.render(markdownContent, env);
...
// ============ .ini AND .zip FILE LINK TESTS ============

test('markdown renderer - .ini file links are converted to absolute paths', async (t) => {
    const sitemap = createTestSitemap();
    const md = createMarkdownInstance({ sitemap });

    const markdownContent = 'Скопировать файл [.usb.ini](.usb.ini) на флешку';
    const env = { page: { inputPath: './external/voyahchat-content/common/usb-debugging.md' } };

    const html = md.render(markdownContent, env);

    t.true(
        html.includes('href=/.usb.ini'),
        `.ini file link should be converted to absolute path, got: ${html}`,
    );
});

test('markdown renderer - .zip file links are converted to absolute paths', async (t) => {
    const sitemap = createTestSitemap();
    const md = createMarkdownInstance({ sitemap });

    const markdownContent = 'Разархивировать [usb-ini.zip](usb-ini.zip) на флешку';
    const env = { page: { inputPath: './external/voyahchat-content/common/usb-debugging.md' } };

    const html = md.render(markdownContent, env);

    t.true(
        html.includes('href=/usb-ini.zip'),
        `.zip file link should be converted to absolute path, got: ${html}`,
    );
});

test('markdown renderer - .ini file links with absolute paths are preserved', async (t) => {
    const sitemap = createTestSitemap();
    const md = createMarkdownInstance({ sitemap });
    ...
    t.true(
        html.includes('href=/.usb.ini'),
        `Absolute .ini file link should be preserved, got: ${html}`,
    );
});

test('markdown renderer - .zip file links with absolute paths are preserved', async (t) => {
    const sitemap = createTestSitemap();
    const md = createMarkdownInstance({ sitemap });
    ...
    t.true(
        html.includes('href=/usb-ini.zip'),
        `Absolute .zip file link should be preserved, got: ${html}`,
    );
});

test('markdown renderer - mixed .ini and .zip file links are handled correctly', async (t) => {
    const sitemap = createTestSitemap();
    const md = createMarkdownInstance({ sitemap });
...
Скопировать файл [.usb.ini](.usb.ini) или разархивировать [usb-ini.zip](usb-ini.zip) на флешку.

Также можно использовать [конфиг](/.usb.ini) или [архив](/usb-ini.zip) с абсолютными путями.
    `.trim();

    const env = { page: { inputPath: './external/voyahchat-content/common/usb-debugging.md' } };
    const html = md.render(markdownContent, env);

    // Check relative paths are converted to absolute
    t.true(html.includes('href=/.usb.ini'), 'Relative .ini file link should be converted to absolute');
    t.true(html.includes('href=/usb-ini.zip'), 'Relative .zip file link should be converted to absolute');

    // Check absolute paths are preserved
    t.true(html.includes('href=/.usb.ini'), 'Absolute .ini file link should be preserved');
    t.true(html.includes('href=/usb-ini.zip'), 'Absolute .zip file link should be preserved');
});

// ============ GITHUB LINK TRANSFORMATION TESTS ============

test('markdown renderer - GitHub links are transformed to local links', async (t) => {
    const sitemap = createTestSitemap();
    const md = createMarkdownInstance({ sitemap });

    const githubBaseUrl = 'https://github.com/voyahchat/voyahchat-install/raw/refs/heads/main/';

    const markdownContent = `
# Test GitHub Links

Download [voyahchat-dns-default.zip](${githubBaseUrl}voyahchat-dns-default.zip)

Or for Mac: [voyahchat-dns-default-mac.zip](${githubBaseUrl}voyahchat-dns-default-mac.zip)

Code example:
\`\`\`
curl -O ${githubBaseUrl}voyahchat-dns-default-mac.zip
\`\`\`

Inline code: \`curl -O ${githubBaseUrl}voyahchat-dns-default.zip\`
    `.trim();

    const env = { page: { inputPath: './external/voyahchat-content/common/software_dns.md' } };
    const html = md.render(markdownContent, env);

    // All zip files should be transformed to full URLs
    t.true(
        html.includes('href=https://voyahchat.ru/voyahchat-dns-default.zip'),
        `Regular zip file should be transformed to full URL, got: ${html}`,
    );

    // Mac zip files should also be transformed to full URLs
    t.true(
        html.includes('href=https://voyahchat.ru/voyahchat-dns-default-mac.zip'),
        `Mac zip file should be transformed to full URL, got: ${html}`,
    );

    // Code blocks should also transform the links
    t.true(
        html.includes('curl -O https://voyahchat.ru/voyahchat-dns-default-mac.zip'),
        `Code block should transform Mac zip to full URL, got: ${html}`,
    );

    // Inline code should also transform the links
    t.true(
        html.includes('curl -O https://voyahchat.ru/voyahchat-dns-default.zip'),
        `Inline code should transform regular zip to full URL, got: ${html}`,
    );
});

// ============ SHARED FILE LINK RESOLUTION TESTS ============

test('markdown renderer - shared files resolve to correct year URL based on context', async (t) => {
    const sitemap = createMockSitemap({
        pages: [
            { url: '/free/parts', title: 'Parts', name: 'Parts', file: 'free/parts.md', section: 'free' },
            { url: '/free/parts/2021', title: '2021', name: '2021', file: 'free/parts_2021.md', section: 'free' },
            { url: '/free/parts/2023', title: '2023', name: '2023', file: 'free/parts_2023.md', section: 'free' },
            {
                url: '/free/parts/2021/powertrain',
                title: 'Powertrain 2021',
                name: 'Powertrain 2021',
                file: 'free/parts_2021-2023_powertrain.md',
                section: 'free',
            },
            {
                url: '/free/parts/2023/powertrain',
                title: 'Powertrain 2023',
                name: 'Powertrain 2023',
                file: 'free/parts_2021-2023_powertrain.md',
                section: 'free',
            },
            {
                url: '/free/parts/2021/fuel',
                title: 'Fuel 2021',
                name: 'Fuel 2021',
                file: 'free/parts_2021-2023_fuel.md',
                section: 'free',
            },
            {
                url: '/free/parts/2023/fuel',
                title: 'Fuel 2023',
                name: 'Fuel 2023',
                file: 'free/parts_2021-2023_fuel.md',
                section: 'free',
    ...
    // From 2021 page: link should resolve to 2021
    const env2021 = {
        page: { inputPath: './external/voyahchat-content/free/parts_2021.md' },
        _currentUrl: '/free/parts/2021',
    };
    const html2021 = md.render('[Powertrain](parts_2021-2023_powertrain.md)', env2021);
    t.true(
        html2021.includes('href=/free/parts/2021/powertrain'),
        `Link from 2021 page should resolve to 2021 URL, got: ${html2021}`,
    );

    // From 2023 page: link should resolve to 2023
    const env2023 = {
        page: { inputPath: './external/voyahchat-content/free/parts_2023.md' },
        _currentUrl: '/free/parts/2023',
    };
    const html2023 = md.render('[Powertrain](parts_2021-2023_powertrain.md)', env2023);
    t.true(
        html2023.includes('href=/free/parts/2023/powertrain'),
        `Link from 2023 page should resolve to 2023 URL, got: ${html2023}`,
    );

    // With anchor from 2021 page
    const html2021Anchor = md.render(
        '[Fuel](parts_2021-2023_fuel.md#some-anchor)',
        env2021,
    );
    t.true(
        html2021Anchor.includes('href=/free/parts/2021/fuel#some-anchor'),
        `Link with anchor from 2021 page should resolve to 2021 URL, got: ${html2021Anchor}`,
    );
});

test('findClosestUrl - picks URL with longest common prefix', (t) => {
    t.is(
        findClosestUrl(['/free/parts/2021/powertrain', '/free/parts/2023/powertrain'], '/free/parts/2021'),
        '/free/parts/2021/powertrain',
        'Should pick 2021 URL when current page is in 2021',
    );

    t.is(
        findClosestUrl(['/free/parts/2021/powertrain', '/free/parts/2023/powertrain'], '/free/parts/2023'),
        '/free/parts/2023/powertrain',
        'Should pick 2023 URL when current page is in 2023',
    );

    t.is(
        findClosestUrl(['/free/parts/2021/powertrain'], '/free/parts/2023'),
        '/free/parts/2021/powertrain',
        'Should return the only URL when array has one element',
    );

    t.is(
        findClosestUrl(
            ['/free/parts/2024/powertrain', '/free/parts/2025/powertrain'],
            '/free/parts/2025',
        ),
        '/free/parts/2025/powertrain',
        'Should pick 2025 URL when current page is in 2025',
    );
});
