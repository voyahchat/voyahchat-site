...
test.before(async (t) => {
    ...
    // Start the server with npm start (build + serve)
    try {
...
test('nginx - compression build compressed files decompress correctly', async (t) => {
    ...
    const originalPath = path.join(siteDir, 'index.html');
...
test('nginx - content-type SVG files have correct MIME type', async (t) => {
    const buildImages = JSON.parse(fs.readFileSync('.build/build-images.json', 'utf8'));
    const svgFiles = Object.entries(buildImages).filter(([filename]) => filename.endsWith('.svg'));
...
test('nginx - content verification CSS brotli response matches original', async (t) => {
    ...
    const originalPath = path.join(siteDir, filename);
...
test('nginx - content verification SVG gzip HTTP response matches disk .gz file exactly', async (t) => {
    ...
    // Read the actual gzip file from disk
    const diskContent = fs.readFileSync(path.join(gzipDir, `${filename}.gz`));

    // Compare raw bytes
    t.deepEqual(response.rawBody, diskContent, `SVG gzip HTTP response should match disk ${filename}.gz exactly`);
});
