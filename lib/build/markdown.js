...
function createMarkdownInstance(options = {}) {
    ...
    md.renderer.rules.code_block = function(tokens, idx, _options, _env, _renderer) {
        ...
        // Проверка: если assets пустой, это может быть ошибка
        if (Object.keys(assets).length === 0 && process.env.NODE_ENV !== 'test') {
            console.warn('Warning: assets.json is empty or not loaded. GitHub URLs will not be replaced.');
        }
