...
function galleryPlugin(md, pluginOptions) {
    md.core.ruler.before('block', 'gallery', function(state) {
...
function renderGallery(images) {
    ...
    for (let idx = 0; idx < images.length; idx++) {
        ...
        html += `<a class="gallery__item" href="${img.src}" data-group="${groupId}">`
            + `<img class="gallery__img" src="${img.src}" alt="${img.alt}"${loading}>`
            + '</a>';
    }
