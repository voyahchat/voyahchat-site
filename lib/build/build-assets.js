...
class AssetsBuilder {
    ...
    async copyAssets(stats) {
        ...
        for (const repo of this.SOURCE_REPOS) {
            try {
                ...
                this.missingRepos.push(repo.name);
                if (process.env.NODE_ENV !== 'test') {
    ...
    async copyVideoAssets(stats) {
        ...
        for (const repo of this.SOURCE_REPOS) {
            try {
                await fs.access(repo.path);
                const videoFiles = await this.findVideoFiles(repo.path);
                ...
                for (const fileInfo of videoFiles) {
                    ...
                    const destPath = path.join(this.siteDir, hashedFilename);

                    await fs.copyFile(fileInfo.fullPath, destPath);

                    try {
                        await Timestamp.setTimestamp(destPath, fileInfo.fullPath);
                    } catch (timestampError) {
                        throw new Error(
                            `CRITICAL: Failed to set timestamp for ${hashedFilename}: `
                            + `${timestampError.message}`,
                        );
                    }

                    hashMap[fileInfo.filename] = hashedFilename;

                    const fileStats = await fs.stat(fileInfo.fullPath);
                    const relativeSourcePath = path.relative(this.projectRoot, fileInfo.fullPath);

                    stats.add(hashedFilename, relativeSourcePath, fileStats.size, {
                        url: `/${hashedFilename}`,
                        type: 'mp4',
                    });
                }
        ...
        const hashVideoPath = path.join(this.dir.getBuild(), 'hash-video.json');

        await fs.writeFile(hashVideoPath, JSON.stringify(hashMap, null, 2) + '\n');
    }
    ...
    async build() {
        ...
        // Copy video assets with content-hash filenames
        await this.copyVideoAssets(stats);
        ...
        if (!this.options.skipWrite) {
            ...
            const meta = {
                availableRepos: this.SOURCE_REPOS
                    .filter(r => !this.missingRepos.includes(r.name))
                    .map(r => r.name),
