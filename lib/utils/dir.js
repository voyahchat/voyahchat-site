...
class Dir {
    ...
    /**
     * Get site HTML directory path
     * @returns {string} Path to site/html directory
     */
    static getSiteHtml() {
        return path.join(Dir.getSite(), 'html');
    }

    /**
     * Get site brotli compression directory path
     * @returns {string} Path to site/brotli directory
     */
    static getSiteBrotli() {
        return path.join(Dir.getSite(), 'brotli');
    }

    /**
     * Get site gzip compression directory path
     * @returns {string} Path to site/gzip directory
     */
    static getSiteGzip() {
        return path.join(Dir.getSite(), 'gzip');
    }

    /**
     * Get build file path by name
     * @param {string} filename - Build file name (e.g., 'build-html.json')
     * @returns {string} Path to build file
     */
    static getBuildFile(filename) {
        return path.join(Dir.getBuild(), filename);
    }

    /**
     * Get external content directory path
     * @returns {string} Path to external/voyahchat-content directory
     */
    static getExternalContent() {
        return path.join(Dir.getRoot(), 'external', 'voyahchat-content');
    }
