...
test('getLastModFromGit() - should return date for existing file', async (t) => {
    const result = await getLastModFromGit('README.md');
...
test('getLastModFromGit() - should return null for non-existent file', async (t) => {
    const result = await getLastModFromGit('non-existent-file-12345.md');
...
test('getLastModFromGit() - should return valid dates for all sitemap files', async (t) => {
    ...
    for (const file of sampleFiles) {
        const lastmod = await getLastModFromGit(file);

        if (lastmod !== null) {
            // Should be in YYYY-MM-DD format
            t.regex(lastmod, /^\d{4}-\d{2}-\d{2}$/, `File ${file} should have valid date format`);

            // Verify the date matches Git
            try {
                const contentDir = path.join(Dir.getRoot(), 'external', 'voyahchat-content');
                const cmd = `cd "${contentDir}" && git log -1 --format=%cI -- "${file}"`;
                const gitDate = execSync(cmd, { encoding: 'utf8' }).trim();

                if (gitDate) {
                    const expectedDate = new Date(gitDate).toISOString().split('T')[0];

                    t.is(lastmod, expectedDate, `Date for ${file} should match Git commit date`);
                }
            } catch (error) {
                // Git command failed, skip verification
                t.pass(`Could not verify Git date for ${file}`);
            }
        }
    }
...
test('SitemapBuilder.build() - builds sitemap from valid config', async (t) => {
    ...
    const contentDir = path.join(testDir, 'external', 'voyahchat-content');

    // Setup: Create valid sitemap.yml
    await fsPromises.mkdir(configDir, { recursive: true });
    await fsPromises.mkdir(contentDir, { recursive: true });
    await fsPromises.writeFile(
        path.join(configDir, 'sitemap.yml'),
        'sitemap:\n  - Home [/, index.md]\n  - About [/about, about.md]\n',
    );

    // Create markdown files
    await fsPromises.writeFile(path.join(contentDir, 'index.md'), '# Home\n\nWelcome home.');
    await fsPromises.writeFile(path.join(contentDir, 'about.md'), '# About\n\nAbout us.');

    // Execute
    const builder = new SitemapBuilder({
        sitemapPath: path.join(configDir, 'sitemap.yml'),
        outputPath: path.join(buildDir, 'sitemap.json'),
    });

    // Override Dir methods for test
    ...
    Dir.getSite = () => path.join(testDir, 'site');
...
test('SitemapBuilder.build() - creates sitemap.json with correct structure', async (t) => {
    ...
    await fsPromises.writeFile(path.join(contentDir, 'test.md'), '# Test\n\nTest content.');
...
test('SitemapBuilder.build() - processes hierarchical navigation correctly', async (t) => {
    ...
    // Setup with hierarchical structure
    await fsPromises.mkdir(configDir, { recursive: true });
    await fsPromises.mkdir(contentDir, { recursive: true });
    await fsPromises.writeFile(
        path.join(configDir, 'sitemap.yml'),
    ...
    await fsPromises.writeFile(path.join(contentDir, 'index.md'), '# Home');
    await fsPromises.mkdir(path.join(contentDir, 'docs'), { recursive: true });
    await fsPromises.writeFile(path.join(contentDir, 'docs/index.md'), '# Docs');
    await fsPromises.writeFile(path.join(contentDir, 'docs/guide.md'), '# Guide');
...
test('SitemapBuilder.build() - handles different URL formats', async (t) => {
    ...
    // Setup with absolute and relative URLs
    await fsPromises.mkdir(configDir, { recursive: true });
    await fsPromises.mkdir(contentDir, { recursive: true });
    await fsPromises.writeFile(
        path.join(configDir, 'sitemap.yml'),
    ...
    await fsPromises.writeFile(path.join(contentDir, 'index.md'), '# Home');
    await fsPromises.mkdir(path.join(contentDir, 'section'), { recursive: true });
    await fsPromises.writeFile(path.join(contentDir, 'section/index.md'), '# Section');
    await fsPromises.writeFile(path.join(contentDir, 'section/relative.md'), '# Relative');
    await fsPromises.writeFile(path.join(contentDir, 'absolute.md'), '# Absolute');
...
test('SitemapBuilder.build() - handles invalid YAML syntax', async (t) => {
    ...
    try {
        const result = await builder.build();
        // Should handle gracefully - invalid lines are skipped with warnings
        t.truthy(result);
        t.true(Array.isArray(result.sitemap));
    } finally {
        Dir.getRoot = originalGetRoot;
...
test('SitemapBuilder.build() - handles missing markdown files gracefully', async (t) => {
    ...
    try {
        ...
        // Should complete but page will have empty HTML
        t.truthy(result);
        t.truthy(result.pages['/']);
...
test('SitemapBuilder.build() - handles circular references in navigation', async (t) => {
    ...
    // Create markdown files with circular link references
    await fsPromises.writeFile(
...
test('SitemapBuilder.build() - sitemap array is correctly populated', async (t) => {
    ...
    await fsPromises.writeFile(path.join(contentDir, 'index.md'), '# Home');
    await fsPromises.writeFile(path.join(contentDir, 'about.md'), '# About');
    await fsPromises.writeFile(path.join(contentDir, 'contact.md'), '# Contact');
...
test('SitemapBuilder.build() - md2url and url2md mappings are accurate', async (t) => {
    ...
    await fsPromises.writeFile(path.join(contentDir, 'index.md'), '# Home');
    await fsPromises.mkdir(path.join(contentDir, 'docs'), { recursive: true });
    await fsPromises.writeFile(path.join(contentDir, 'docs/guide.md'), '# Docs');
...
test('SitemapBuilder.build() - handles very deep navigation hierarchy', async (t) => {
    ...
    await fsPromises.writeFile(path.join(contentDir, 'l1.md'), '# L1');
    await fsPromises.writeFile(path.join(contentDir, 'l2.md'), '# L2');
    await fsPromises.writeFile(path.join(contentDir, 'l3.md'), '# L3');
    await fsPromises.writeFile(path.join(contentDir, 'l4.md'), '# L4');
    await fsPromises.writeFile(path.join(contentDir, 'l5.md'), '# L5');
...
test('SitemapBuilder.build() - handles special characters in URLs', async (t) => {
    ...
    await fsPromises.writeFile(
        path.join(configDir, 'sitemap.yml'),
        'sitemap:\n  - Cyrillic [/cyrillic, test.md]\n  - FAQ [/faq-2024, faq.md]\n',
    );

    await fsPromises.writeFile(path.join(contentDir, 'test.md'), '# Cyrillic');
    await fsPromises.writeFile(path.join(contentDir, 'faq.md'), '# FAQ');
...
test('SitemapBuilder.build() - skipWrite option prevents file creation', async (t) => {
    ...
    try {
        ...
        // Should return result but not write files
        t.truthy(result);
        t.false(fileExists(path.join(buildDir, 'sitemap.json')));
        t.false(fileExists(path.join(testDir, 'site', 'sitemap.xml')));
    } finally {
        Dir.getRoot = originalGetRoot;
        Dir.getBuild = originalGetBuild;
        Dir.getSite = originalGetSite;
...
test('SitemapBuilder - recovers from corrupted sitemap.yml mid-processing', async (t) => {
    ...
    try {
        ...
        // Create partially corrupted sitemap.yml (valid YAML but invalid format)
        await fsPromises.writeFile(
            path.join(configDir, 'sitemap.yml'),
            'sitemap:\n' +
            '  - Valid [/, index.md]\n' +
        ...
        await fsPromises.writeFile(path.join(contentDir, 'index.md'), '# Home');
        await fsPromises.writeFile(path.join(contentDir, 'valid.md'), '# Valid');
        ...
        // Should handle corrupted lines gracefully by skipping them
        const result = await builder.build();
...
test('SitemapBuilder - cleans up after sitemap build failure', async (t) => {
    ...
    const buildFiles = await fsPromises.readdir(buildDir).catch(() => []);
    const siteFiles = await fsPromises.readdir(siteDir).catch(() => []);
...
test('SitemapBuilder - handles circular dependencies without infinite loops', async (t) => {
    ...
    try {
        ...
        // Create sitemap with potential circular structure
        await fsPromises.writeFile(
            path.join(configDir, 'sitemap.yml'),
