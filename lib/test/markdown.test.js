...
More text with \`0107  — месяц, день\` and — em dash.`;
    ...
    t.true(html.includes('<code class="article__inline-code">2026  — год</code>'));
    t.true(html.includes('<code class="article__inline-code">0107  — месяц, день</code>'));

    // Verify no typography in inline code
    t.false(html.includes('<code class="article__inline-code">2026\\u00A0— год</code>'));
    t.false(html.includes('<code class="article__inline-code">0107\\u00A0— месяц, день</code>'));
});
...
- Final item with — em dash`;
    ...
    t.true(html.includes('<code class="article__inline-code">code with — dash</code>'));
    t.false(html.includes('<code class="article__inline-code">code with\\u00A0— dash</code>'));

    // Count occurrences to ensure we have the right balance
    const regularTypographyCount = (html.match(/\u00A0— /g) || []).length;
    const codeWithoutTypographyCount = (
        html.match(/<code class="article__inline-code">[^<]*—[^<]*<\/code>/g) || []
    ).length;
...
test('Markdown rendering - should preserve space between code and link', (t) => {
    ...
    t.true(
        html.includes('<code class="article__inline-code">10000</code> <a'),
        'Should have space between code and link',
    );
});
