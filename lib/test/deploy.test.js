...
class MockFTPClient {
    ...
    async list(path = '.') {
        const items = [];
        for (const [remotePath, file] of this.files.entries()) {
            const itemPath = path === '.' ? remotePath : remotePath.substring(path.length + 1);
            if (itemPath && !itemPath.includes('/')) {
                ...
                itemPath.includes('/') &&
                !itemPath.substring(itemPath.indexOf('/') + 1).includes('/')) {
                const dirName = itemPath.split('/')[0];
                if (!items.find(i => i.name === dirName)) {
                    items.push({
                        name: dirName,
                        type: 'd',
                        size: 0,
...
test('FTP.constructor() - should create instance with default config path', t => {
    const ftp = new FTP('config/ftp.yml', t.context.testDir);
...
test('FTP.constructor() - should accept custom config path', t => {
    const ftp = new FTP('custom/config.yml', t.context.testDir);
...
`;
    ...
    const ftp = new FTP(configPath, t.context.testDir);
...
test('FTP.loadConfig() - should throw error for invalid file', t => {
    const ftp = new FTP('nonexistent.yml', t.context.testDir);
    t.throws(() => ftp.loadConfig(), {
        message: /Failed to load FTP config/,
    });
});

test('FTP.loadConfig() - should load config from environment variables', t => {
    // Set environment variables
    process.env.FTP_HOST = 'ftp.example.com';
    process.env.FTP_USER = 'testuser';
    process.env.FTP_PASS = 'testpass';
    process.env.FTP_PATH = '/test';
...
test('FTP.loadConfig() - should fallback to file when env vars are incomplete', t => {
    // Set only some environment variables (missing FTP_PASS)
    process.env.FTP_HOST = 'ftp.example.com';
    process.env.FTP_USER = 'testuser';
    process.env.FTP_PATH = '/test';
...
`;
    ...
    // Should load from file, not environment
    t.is(config.host, 'ftp.file.com');
    t.is(config.user, 'fileuser');
    t.is(config.password, 'filepass');
    t.is(config.remote_path, '/file');
...
`;
    ...
    // Generate timestamps.json using TestDir
...
`;
    ...
    // Generate local timestamps in test directory
    new TimestampsBuilder(t.context.testDir).generate();
    ...
    // Mock syncServerTimestamps to return server timestamps
    const originalSync = ftp.syncServerTimestamps;
    ftp.syncServerTimestamps = async () => {
        const testTimestampsPath = path.join(siteDir, 'timestamps-server.json');
        fs.writeFileSync(testTimestampsPath, JSON.stringify(serverTimestamps));
    ...
    ftp.syncServerTimestamps = originalSync;
});

test('FTP.getOrphanedFiles() - should find files on server but not locally', t => {
    const ftp = new FTP(null, t.context.testDir);
    const localTimestamps = { 'html/index.html': 1000, 'css/style.css': 2000 };
    const serverTimestamps = {
        'html/index.html': 1000,
        'css/style.css': 2000,
        'html/old.html': 3000,
        'jpg/removed.jpg': 4000,
    };
    const result = ftp.getOrphanedFiles(localTimestamps, serverTimestamps);
    t.deepEqual(result, ['html/old.html', 'jpg/removed.jpg']);
});

test('FTP.getOrphanedFiles() - should protect timestamps files from deletion', t => {
    const ftp = new FTP(null, t.context.testDir);
    const localTimestamps = { 'html/index.html': 1000 };
    const serverTimestamps = {
        'html/index.html': 1000,
        'timestamps.json': 5000,
        'timestamps-server.json': 5000,
        'html/old.html': 3000,
    };
    const result = ftp.getOrphanedFiles(localTimestamps, serverTimestamps);
    t.deepEqual(result, ['html/old.html']);
});

test('FTP.getOrphanedFiles() - should return empty array when no orphans', t => {
    const ftp = new FTP(null, t.context.testDir);
    const localTimestamps = { 'html/index.html': 1000 };
    const serverTimestamps = { 'html/index.html': 1000 };
    const result = ftp.getOrphanedFiles(localTimestamps, serverTimestamps);
    t.deepEqual(result, []);
});

test('FTP.listRemoteFiles() - should list all files recursively', async t => {
    const ftp = new FTP('config/ftp.yml', t.context.testDir);
    ftp.client = t.context.mockClient;

    const testDate = new Date();
    t.context.mockClient.files.set('file1.txt', {
        content: Buffer.from('content1'),
        timestamp: testDate,
    });
    t.context.mockClient.files.set('dir/file2.txt', {
        content: Buffer.from('content2'),
        timestamp: testDate,
    });

    // Override list to properly simulate FTP LIST for nested directories
    let callCount = 0;
    ftp.client.list = async (path) => {
        callCount++;
        if (path === '.' || callCount === 1) {
            return [
                { name: 'file1.txt', type: '0', size: 8, modified: testDate },
                { name: 'dir', type: 'd', size: 0, modified: testDate, isDirectory: true },
            ];
        }
        return [
            { name: 'file2.txt', type: '0', size: 8, modified: testDate },
        ];
    };

    const files = await ftp.listRemoteFiles('.');

    t.is(files.length, 2);
    t.true(files.some(f => f.path === 'file1.txt'));
    t.true(files.some(f => f.path === 'dir/file2.txt'));
});

test('FTP.syncServerTimestamps() - should create timestamps-server.json from FTP', async t => {
    const configPath = path.join(t.context.testDir.getConfig(), 'ftp.yml');
...
`);
    ...
    t.context.mockClient.files.set('file2.zip', {
        content: Buffer.from('zip'),
        timestamp: testDate,
    });

    ftp.client.list = async () => [
        { name: 'file1.txt', type: '0', size: 8, modified: testDate },
        { name: 'file2.zip', type: '0', size: 3, modified: testDate },
    ];

    await ftp.syncServerTimestamps({ progress: false });

    const siteDir = t.context.testDir.getSite();
    const serverTimestampsPath = path.join(siteDir, 'timestamps-server.json');
    t.true(fs.existsSync(serverTimestampsPath));

    const timestamps = JSON.parse(fs.readFileSync(serverTimestampsPath, 'utf8'));
    t.is(Object.keys(timestamps).length, 2);
    t.true(typeof timestamps['file1.txt'] === 'number');
    t.true(typeof timestamps['file2.zip'] === 'number');
});
