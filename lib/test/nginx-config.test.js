...
test('nginx config() - blocks compression directories', async (t) => {
    ...
    t.true(config.includes('location ~ ^/(brotli|gzip)'));
...
test('nginx config() - checks for compressed file existence', async (t) => {
    ...
    t.true(config.includes('if (-f $document_root/$compression_dir'));
    t.true(config.includes('$compression_suffix)'));
    t.true(config.includes('set $file_path /$compression_dir'));
});
