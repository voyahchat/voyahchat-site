 ...
 * Processes templates from blocks/ directory.
 *
...
class TemplateOptimizer {
    ...
    /**
     * Find all template files in blocks directory
     *
    ...
    async findTemplates(rootDir) {
        ...
        return this.findFilesRecursive(blocksDir, '.njk');
    }
    ...
    removeOptionalTags(html) {
        ...
        // Remove </li> before </ul> (final list item)
        html = html.replace(/<\/li>\s*<\/ul>/gi, '</ul>');

        // Remove </p> in list items
        html = html.replace(/(<li[^>]*>.*?)<\/p>(?=.*?(?:<\/li>|<\/ul>))/gi, '$1');
