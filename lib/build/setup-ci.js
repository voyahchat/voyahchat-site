...
function updateNginxConfig() {
    console.log('\nUpdating nginx configuration for CI environment...');

    const nginxConfigPath = path.join(__dirname, '../../config/config-nginx.njk');

    try {
        // Read the current nginx configuration
        let configContent = fs.readFileSync(nginxConfigPath, 'utf8');

        // Replace the macOS Homebrew path with Ubuntu path
        ...
        fs.writeFileSync(nginxConfigPath, configContent, 'utf8');

        console.log('  Nginx configuration updated for CI environment');
    } catch (error) {
...
function main() {
    ...
    try {
        ...
        // Update nginx configuration for CI environment
        updateNginxConfig();
