...
test.afterEach.always(async (t) => {
    ...
    if (t.context.dir) {
        await cleanupTestDir(t.context.dir);
    }
});
    ...
    builder.SOURCE_REPOS = [
        {
            ...
            path: path.join(t.context.testDir, 'nonexistent'),
            baseUrl: 'https://github.com/test/nonexistent/raw/refs/heads/main',
        },
    ];

    // Should not throw error but return empty asset map
    const assetMap = await builder.copyAssets();
    t.deepEqual(assetMap, {}, 'Should return empty asset map for missing directory');
});
...
test('AssetsBuilder.copyAssetsToSite() - should copy assets to site directory', async (t) => {
    const builder = new AssetsBuilder({ skipWrite: false }); // Need to actually copy files
    const testAssetsDir = t.context.testDir;
    ...
    await builder.copyAssetsToSite(assetsJson, stats);
...
test('AssetsBuilder() - should handle missing external repos gracefully', async (t) => {
    const testAssetsDir = t.context.testDir;
    const testSiteDir = path.join(t.context.testDir, 'site');
    ...
    // Should handle missing directory gracefully and return empty asset map
    const assetMap = await builder.copyAssets();
    t.deepEqual(assetMap, {}, 'Should return empty asset map for missing directory');
});
...
test('AssetsBuilder() - should handle permission errors during file copy', async (t) => {
    ...
    // In real implementation, permission errors are caught and logged, not thrown
    // So we just verify the method completes without throwing
...
test('AssetsBuilder.build() - should include .htaccess in config files count', async (t) => {
    ...
    await fs.writeFile(path.join(assetsDir, 'assets.json'), '{}', 'utf8');

    const builder = new AssetsBuilder({ skipWrite: false });
    builder.projectRoot = testRoot;
    builder.CONFIG_DIR = configDir;
    builder.ASSETS_DIR = assetsDir;
    builder.siteDir = siteDir;
    builder.ASSETS_JSON_PATH = path.join(assetsDir, 'assets.json');
