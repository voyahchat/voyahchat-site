...
async function validateMarkdownFile(filePath) {
    try {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            // Update context for the current line
            ...
            if (isListLine(line)) {
                ...
                const indentationViolation = validateListIndentation(line, lineNumber);
...
function isListLine(line) {
    // Check for lines that start with optional spaces followed by - or *
 ...
 */
function validateListIndentation(line, _lineNumber) {
    ...
    // Rule 2: Nested lists must use exactly 4-space multiples (4, 8, 12 spaces = valid)
    // Invalid nested indentation: 5, 6, 7, 9, 10, 11, 13 spaces = violations
    if (leadingSpaces > 0) {
