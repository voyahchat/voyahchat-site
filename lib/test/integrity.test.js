        ...
        while ((match = linkRegex.exec(content)) !== null) {
            ...
            if (anchor) {
                ...
                const htmlFilename = normalizedUrl === '/'
                    ? 'index.html'
                    : `${normalizedUrl.slice(1).replace(/\//g, '_')}.html`;
...
test('HTML quality - W3C HTML validation', async (t) => {
    ...
    const validationPromises = Object.entries(buildHtml).map(async ([filename]) => {
        const filePath = path.join(siteDir, filename);
    ...
    t.pass(`All ${validationResults.length} HTML files passed W3C validation`);
});
