...
class TimestampsJson {
    /**
     * Write timestamps object to JSON file with consistent formatting
     * Keys are sorted alphabetically, optional excludeBuildTime parameter
     */
    static writeTimestamps(timestamps, filePath, excludeBuildTime = false) {
        const sorted = {};
        Object.keys(timestamps).sort().forEach(key => {
            if (excludeBuildTime && key === 'buildTime') {
                return;
            }
            sorted[key] = timestamps[key];
        });
    ...
    static compareTimestamps(localTimestamps, remoteTimestamps, options = {}) {
        ...
        // Check if sitemap.xml has changed
        const sitemapPath = 'sitemap.xml';
        const localSitemapTimestamp = localTimestamps[sitemapPath];
        const remoteSitemapTimestamp = remoteTimestamps[sitemapPath];
        const sitemapChanged = localSitemapTimestamp && remoteSitemapTimestamp &&
            localSitemapTimestamp > remoteSitemapTimestamp &&
            Math.abs(localSitemapTimestamp - remoteSitemapTimestamp) > timeDiffTolerance;
        ...
        for (const [filePath, localTimestamp] of Object.entries(localTimestamps)) {
            ...
            if (!remoteTimestamp) {
                ...
                } else if (sitemapChanged && filePath.endsWith('.html') && !filePath.endsWith('.zip')) {
                    // If sitemap changed, force re-upload all HTML files
                    filesToUpload.push({
                        path: filePath,
                        reason: 'sitemap-updated',
                        localTimestamp,
                        serverTimestamp: remoteTimestamp,
                    });
                    if (verbose) {
                        console.log(`  [SITEMAP] ${filePath}: sitemap changed, re-uploading`);
                    }
                } else {
