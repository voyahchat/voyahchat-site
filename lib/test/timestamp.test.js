...
test('build-manifest - NO files should have build-time timestamps', async (t) => {
    ...
    // Use manifest.json mtime as build time reference
    const manifestPath = path.join(siteDir, 'manifest.json');
    if (!fs.existsSync(manifestPath)) {
        t.fail('manifest.json not found - build should create this file');
        return;
    }
    ...
    try {
        ...
        buildStartTime = Math.floor(manifestStats.mtimeMs / 1000);
    } catch (error) {
        t.fail(`Failed to read manifest.json: ${error.message}`);
        return;
    }
