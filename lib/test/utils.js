...
/**
 * Create a file in TestDir with content
 * @param {TestDir} dir - TestDir instance
 * @param {string} relativePath - Relative path from TestDir root
 * @param {string} content - File content
 * @returns {Promise<string>} Full path to created file
 */
async function createTestFile(dir, relativePath, content = '') {
    const fullPath = path.join(dir.getRoot(), relativePath);
...
/**
 * Create markdown files in TestDir content directory
 * @param {TestDir} dir - TestDir instance
 * @param {Object} files - Object with relative paths (from content dir) as keys
 * @returns {Promise<Object>} Object with full paths as keys
 */
async function createTestContent(dir, files) {
    const contentFiles = {};

    await Promise.all(Object.entries(files).map(async ([relativePath, content]) => {
        const fullPath = path.join(dir.getContent(), relativePath);
        await fs.promises.mkdir(path.dirname(fullPath), { recursive: true });
        await fs.promises.writeFile(fullPath, content, 'utf8');
        contentFiles[relativePath] = fullPath;
    }));
...
/**
 * Create sitemap.yml file in TestDir
 * @param {TestDir} dir - TestDir instance
 * @param {string|Array} sitemapContent - Sitemap content (string YAML or array structure)
 * @returns {Promise<string>} Path to created sitemap file
 */
async function createTestSitemap(dir, sitemapContent) {
    let yamlContent;

    if (Array.isArray(sitemapContent)) {
        yamlContent = 'sitemap:\n' + sitemapContent.map(item => `  - ${item}`).join('\n');
    } else {
        // If content doesn't already start with 'sitemap:', add it
        if (sitemapContent.trim().startsWith('sitemap:')) {
            yamlContent = sitemapContent;
        } else {
            yamlContent = 'sitemap:\n  - ' + sitemapContent;
        }
    }

    return createTestFile(dir, 'config/sitemap.yml', yamlContent);
}

/**
 * Create builder with TestDir for unit tests
 * @param {Class} BuilderClass - Builder class to instantiate
 * @param {Object} options - Builder options
 * @param {TestDir} dir - TestDir instance (creates new one if not provided)
 * @returns {Object} { builder, dir }
 */
function createTestBuilder(BuilderClass, options = {}, dir = null) {
    const { TestDir } = require('./test-dir');
    const testDir = dir || new TestDir();
    const builder = new BuilderClass(options, testDir);
    return { builder, dir: testDir };
}
 ...
 *
 * @param {TestDir} dir - TestDir instance (optional, uses global Dir if not provided)
...
function getSitemap(dir) {
    try {
        ...
        if (dir && dir.getBuild) {
            // Use TestDir if provided
            sitemapPath = path.join(dir.getBuild(), 'sitemap.json');
        } else {
        ...
        if (!fileExists(sitemapPath)) {
            // In test environment with TestDir, throw error (test expects this)
            if (dir && dir.getBuild) {
 ...
 */
function getImageMapping(dir) {
    let mappingPath;

    if (dir && dir.getBuild) {
        // Use TestDir if provided
        mappingPath = path.join(dir.getBuild(), 'image-mapping.json');
    } else {
        // Fallback to global Dir
        mappingPath = path.join(Dir.getBuild(), 'image-mapping.json');
    }
...
function createMockSitemap(options = {}) {
    ...
    pages.forEach((page) => {
        ...
        if (page.file) {
            ...
            if (!md2urls[page.file]) {
                md2urls[page.file] = [];
            }
            md2urls[page.file].push(page.url);
            url2md[page.url] = page.file;
        }
    });

    return { sitemap, pages: pagesMap, md2url, md2urls, url2md };
}
