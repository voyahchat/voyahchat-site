...
function getBuildArtifactPath(filename) {
    return Dir.getBuildFile(filename);
}
...
test.before('Ensure build exists for stats tests', (t) => {
    const buildCssPath = Dir.getBuildFile('build-css.json');
    const siteIndexPath = path.join(Dir.getSiteHtml(), 'index.html');

    // Build artifacts should already exist from npm test script
    if (!fs.existsSync(buildCssPath) || !fs.existsSync(siteIndexPath)) {
...
test('build-css.json - has unified format', (t) => {
    ...
    Object.entries(buildCss).forEach(([filename, info]) => {
        ...
        // CSS files can be in css/ subdirectory or root
        const basename = path.basename(filename);
        t.true(
            basename.startsWith('_c') || basename.endsWith('.css'),
...
test('build-js.json - has unified format', (t) => {
    ...
    Object.entries(buildJs).forEach(([filename, info]) => {
        ...
        assertPositiveNumber(t, info.size, `${filename}.size`);
        // JS files can be in js/ subdirectory or root
        const basename = path.basename(filename);
        t.true(
            basename.startsWith('_j') || basename.endsWith('.js'),
...
test('build-images.json - has unified format', (t) => {
    ...
    Object.entries(buildImages).forEach(([filename, info]) => {
        ...
        // Allow zero size for corrupted/empty files, but require non-negative
        t.true(info.size >= 0, `${filename}.size should be non-negative, got ${info.size}`);

        // Images can be in subdirectories (svg/, png/, jpg/, webp/) or root
        const basename = path.basename(filename);

        // SVG files are saved with _s prefix (no extension), other images have extensions
        if (basename.startsWith('_s')) {
...
test('statistics - all site files are tracked', (t) => {
    ...
    // Add robots.txt and .htaccess as they're copied by build-assets.js
    trackedFiles.add('robots.txt');
    trackedFiles.add('.htaccess');
    ...
    actualFiles.forEach((file) => {
        // Skip timestamps files as they are special files used during deployment
        if (file === 'timestamps-server.json' || file === 'timestamps.json' ||
            file === 'new.pdf') {
            return;
        }

        // For files in subdirectories, check both with and without directory prefix
        // since statistics files track files without directory prefixes
        const basename = path.basename(file);
        const isTracked = trackedFiles.has(file) || trackedFiles.has(basename);
        t.true(isTracked, `File ${file} should be tracked in statistics`);
    });
});
