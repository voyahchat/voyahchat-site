...
class FTP {
    ...
    async fixRemoteTimestamp(remotePath, localPath) {
        try {
            const stats = fs.statSync(localPath);
            const date = stats.mtime;
            const ts = date.toISOString().replace(/[-:]/g, '').replace(/\..+/, '').replace('T', '');
            await this.client.send(`MFMT ${ts} ${remotePath}`);
        } catch {
            // MFMT not supported or failed — non-critical
        }
    }
    ...
    async uploadFile(localPath, remotePath, progress = true, retryCount = 0) {
        ...
        try {
            ...
            await this.fixRemoteTimestamp(remotePath, localPath);
    ...
    async listServerFiles(progress = true) {
        const files = {};
        const listDir = async (dirPath) => {
            let list;
            try {
                list = await this.client.list(dirPath);
            } catch {
                return;
            }
            for (const item of list) {
                ...
                const isDir = item.isDirectory || item.type === 2 || item.type === 'd';
                if (isDir) {
                    await listDir(fullPath);
                } else {
                    files[fullPath] = item.size;
                }
            }
        };
        await listDir('.');
        if (progress) {
            console.log(`Server LIST: ${Object.keys(files).length} files`);
        }
        return files;
    }
    ...
    async deploy(options = {}) {
        ...
        try {
            ...
            if (progress) {
                console.log(`Local: ${localFileCount} files`);
            }

            // Get actual server file sizes via LIST (ground truth) — always, even in dry-run,
            // so orphan detection reflects the real server state.
            let serverFiles = {};
            if (progress) {
                console.log('Scanning server files...');
            }
            serverFiles = await this.listServerFiles(progress);

            // Load server manifest (for hash comparison)
            let serverManifest = {};
            let noServerManifest = false;
            if (!force) {
                try {
                    ...
                    if (progress) {
                        ...
                        console.log(`Server manifest: ${serverCount} files`);
                    }
                } catch (err) {
                    noServerManifest = true;
                    if (progress) {
                        console.log('No server manifest found. Will upload all files.');
                    }
                }
            }

            // Preserve files from missing source repos
            if (!force && !noServerManifest) {
                const buildDir = this.dir.getBuild();
                const metaPath = path.join(buildDir, 'build-assets-meta.json');
                try {
                    const meta = JSON.parse(fs.readFileSync(metaPath, 'utf8'));
                    if (meta.missingRepos && meta.missingRepos.length > 0) {
                        const statsPath = path.join(buildDir, 'build-assets.json');
                        const prevStats = JSON.parse(fs.readFileSync(statsPath, 'utf8'));
                        const missingRepoPrefixes = meta.missingRepos.map(name => `external/${name}/`);
                        const filesFromMissingRepos = Object.entries(prevStats)
                            .filter(([, info]) => info.source
                                && missingRepoPrefixes.some(p => info.source.startsWith(p)))
                            .map(([filename]) => filename);
                        const preserved = Manifest.preserveFiles(localManifest, serverManifest, filesFromMissingRepos);
                        if (preserved > 0 && progress) {
                            const repoList = meta.missingRepos.join(', ');
                            console.log(`Preserved ${preserved} files from missing repos: ${repoList}`);
                        }
                    }
                } catch {
                    // No meta file - build with all repos available, normal behavior
                }
            }

            // Compare: use actual server file sizes from LIST, hash only when sizes match.
            // Dry-run uses the same real comparison so the planned diff is accurate.
            let comparison;
            if (force || noServerManifest) {
                comparison = {
                    toUpload: Object.keys(localManifest).map(p => ({ path: p, reason: force ? 'force' : 'new' })),
                    toDelete: [],
                ...
                comparison = Manifest.compareWithServer(localManifest, serverManifest, serverFiles);
                // Safety limit: abort if too many files would be deleted. Base it on the actual
                // server file count (serverFiles), since orphan detection now uses that listing.
                const serverFileCount = Object.keys(serverFiles).length;
                if (serverFileCount > 0 && comparison.totalToDelete > serverFileCount * 0.3) {
                    throw new Error(
                        `Safety abort: would delete ${comparison.totalToDelete} of ${serverFileCount} files (>30%). ` +
            ...
            // Delete orphaned files (dry-run prints the plan without touching the server)
            if (comparison.totalToDelete > 0) {
                if (progress) {
                    const action = dryRun ? 'Would delete' : 'Deleting';
                    console.log(`\n${action} ${comparison.totalToDelete} orphaned files...`);
                    comparison.toDelete.forEach(f => console.log(`  ${f}`));
                }
            ...
            // Verify via LIST, retry failed uploads, then upload manifest
            if (!dryRun) {
                if (progress) {
                    console.log('Verifying files on server...');
                }
                const verifyFiles = await this.listServerFiles(progress);
                const failedFiles = [];
                for (const [filePath, info] of Object.entries(localManifest)) {
                    if (!(filePath in verifyFiles)) {
                        failedFiles.push({ path: filePath, reason: 'missing' });
                        if (progress) {
                            console.error(`  VERIFY FAILED: ${filePath} not found on server`);
                        }
                    } else if (verifyFiles[filePath] !== info.size) {
                        failedFiles.push({ path: filePath, reason: 'size mismatch' });
                        if (progress) {
                            console.error(
                                `  VERIFY FAILED: ${filePath} size mismatch ` +
                                `(expected ${info.size}, got ${verifyFiles[filePath]})`,
                            );
                        }
                    } else if (progress) {
                        console.log(`  OK ${filePath} (${info.size} bytes)`);
                    }
                }
                const ok = localFileCount - failedFiles.length;
                if (progress) {
                    console.log(`  Verified: ${ok}, Failed: ${failedFiles.length}`);
                }
                ...
                if (failedFiles.length > 0) {
                    ...
                    for (const fail of failedFiles) {
                        const localPath = fileMap.get(fail.path);
                        if (!localPath) {
                            this.stats.errors++;
                            continue;
                        }
                        await this.uploadFile(localPath, fail.path, progress);
                    }
                    ...
                    const reverifyFiles = await this.listServerFiles(progress);
                    let stillMissing = 0;
                    let stillMismatch = 0;
                    for (const fail of failedFiles) {
                        const info = localManifest[fail.path];
                        if (!(fail.path in reverifyFiles)) {
                            stillMissing++;
                            if (progress) {
                                console.error(`  STILL FAILED: ${fail.path} not found on server`);
                            }
                        } else if (reverifyFiles[fail.path] !== info.size) {
                            stillMismatch++;
                            if (progress) {
                                console.error(
                                    `  STILL FAILED: ${fail.path} size mismatch ` +
                                    `(expected ${info.size}, got ${reverifyFiles[fail.path]})`,
                                );
                            }
                        } else if (progress) {
                            console.log(`  OK ${fail.path} (${info.size} bytes)`);
                        }
                    }
                    if (stillMissing > 0 || stillMismatch > 0) {
                        this.stats.errors += stillMissing + stillMismatch;
                        throw new Error(
                            `Verification failed after retry: ${stillMissing} missing, ` +
                            `${stillMismatch} size mismatch`,
                        );
                    }
                    if (progress) {
                        console.log(`All ${failedFiles.length} retried files verified`);
                    }
                } else if (progress) {
                    console.log(`All ${localFileCount} files verified`);
                }

                // Upload manifest last so it always reflects actual server state
