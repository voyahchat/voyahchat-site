...
class FTP {
     ...
     */
    async syncServerTimestamps() {
        const siteDir = this.dir.getSite();
        const serverTimestampsPath = path.join(siteDir, 'timestamps-server.json');

        if (this.config && this.config.progress !== false) {
        ...
        for (const file of remoteFiles) {
            ...
            const showProgress = this.config && this.config.progress !== false;
    ...
    async deploy(options = {}) {
        ...
        try {
            ...
            if (!dryRun && !force) {
                ...
                let serverTimestamps = await this.downloadServerTimestampsFile();
                let usedCache = false;
                ...
                if (!usedCache) {
                    ...
                    serverTimestamps = await this.syncServerTimestamps();
