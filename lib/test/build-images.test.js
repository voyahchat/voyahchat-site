...
test('ImageProcessor() - should processImage handles duplicate content', async (t) => {
    ...
    const outputFile = path.join(testDir, hashedFilename);
...
test('ImageProcessor() - should generateMappingFile creates correct structure with _i prefix', async (t) => {
    ...
    // Mapping should use _i prefix for image references
...
test('ImageProcessor() - should build process completes successfully', async (t) => {
    ...
    await Promise.all(Object.values(result.mapping).map(async (imageRef) => {
        // Extract actual hash from imageRef (_i prefix)
        const actualHash = imageRef.replace(/^_i/, '');
...
test('ImageProcessor() - should generates correct image mapping with _i prefix', async (t) => {
    ...
    t.deepEqual(mappingContent, {
        ...
        'path/to/image2.jpg': '_ihash2',
        'image3.svg': '_ihash3',
    }, 'Mapping should contain all entries with _i prefix');
