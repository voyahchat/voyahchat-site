...
const { getSitemap, getImageMapping } = require('./utils');

const BASE = 'https://github.com/voyahchat/voyahchat-(install|docs)/raw/refs/heads/main/';
const REGEX = new RegExp(BASE + '([^\\s"\\\')]+)', 'g');
...
function transformAssetLinkPath(originalHref) {
    ...
    // Direct transformation for both voyahchat-install and voyahchat-docs repositories
    if (originalHref.match(REGEX)) {
        // Use exec to properly extract the capture group
        const match = REGEX.exec(originalHref);
        // Reset regex lastIndex to ensure it works correctly for subsequent calls
        REGEX.lastIndex = 0;

        if (match && match[2]) {
            const filename = match[2];

            // All files use full URL
            return `https://voyahchat.ru/${filename}`;
        }
    }
...
/**
 * Transform GitHub raw asset links in text content to full URLs
 */
function transformGitHubLinksInText(content) {
    return content.replace(REGEX, (match, repo, filename) => {
        // All files use full URL
        return `https://voyahchat.ru/${filename}`;
    });
}
...
function transformAnchorLink(originalHref, env) {
    ...
    if (env && env._processingState && env._currentUrl) {
        ...
        if (anchorMapping) {
            // Try direct match first, then decode if needed
            let hierarchicalAnchor = anchorMapping.get(githubAnchor);
            if (!hierarchicalAnchor) {
                try {
                    const decodedAnchor = decodeURIComponent(githubAnchor);
                    hierarchicalAnchor = anchorMapping.get(decodedAnchor);
                } catch (error) {
                    // If decoding fails, we already tried direct match
                }
            }
            if (hierarchicalAnchor) {
    ...
    // If not found in mapping, try to resolve forward references by looking ahead in the markdown

    // If still not found and we have a file path, try to resolve by reading the file
    if (env && env.page && env.page.inputPath && fs.existsSync(env.page.inputPath)) {
        try {
            const content = fs.readFileSync(env.page.inputPath, 'utf8');
            const githubSlugFunc = createGitHubSlugify('lower');
            const slugifyFunc = createCyrillicSlugify('lower');

            // Parse the markdown to find headings and their hierarchical structure
            const headingStack = [];
            const lines = content.split('\n');

            // Track occurrences of each anchor to handle GitHub's -1, -2 suffix logic
            const anchorOccurrences = new Map();
            const matchingHierarchicalAnchors = [];
            ...
            for (const line of lines) {
                ...
                if (headingMatch) {
                    ...
                    // Generate GitHub-style anchor from the text
                    const generatedGithubAnchor = githubSlugFunc(text);

                    // Count occurrences for GitHub-style deduplication
                    const baseAnchor = generatedGithubAnchor.replace(/-\d+$/, '');
                    const occurrenceCount = (anchorOccurrences.get(baseAnchor) || 0) + 1;
                    anchorOccurrences.set(baseAnchor, occurrenceCount);

                    // Create the actual GitHub anchor with suffix if needed
                    const actualGithubAnchor = occurrenceCount > 1 ?
                        `${baseAnchor}-${occurrenceCount - 1}` :
                        baseAnchor;

                    // Generate the hierarchical anchor for this heading
                    const currentStack = headingStack.slice(0, level);
                    const hierarchicalAnchor = buildHierarchicalAnchor(currentStack, slugifyFunc);

                    // Store the mapping
                    matchingHierarchicalAnchors.push({
                        githubAnchor: actualGithubAnchor,
                        hierarchicalAnchor: hierarchicalAnchor,
                    });

                    // Also check for version number patterns (e.g., "2.1.2" -> "212")
                    if (/^\d+\.\d+(\.\d+)?$/.test(text.trim())) {
                        const versionAnchor = text.trim().replace(/\./g, '');
                        const versionHierarchicalAnchor = buildHierarchicalAnchor(currentStack, slugifyFunc);

                        matchingHierarchicalAnchors.push({
                            githubAnchor: versionAnchor,
                            hierarchicalAnchor: versionHierarchicalAnchor,
                        });
                    }
                }
            }

            // Find the matching hierarchical anchor for our target
            const match = matchingHierarchicalAnchors.find(item => item.githubAnchor === githubAnchor);
            if (match) {
                // Return the hierarchical anchor if it's different from the GitHub anchor
                // This handles proper transformation from GitHub-style to hierarchical
                return `#${match.hierarchicalAnchor}`;
            }
        } catch (error) {
            // If there's an error reading the file, fall back to original behavior
            console.warn(`Warning: Could not resolve forward reference for anchor "${githubAnchor}": ${error.message}`);
        }
    }
...
/**
 * Find the best URL match for a shared file based on current page context.
 * Picks the URL with the longest common path prefix relative to the current page.
 * @param {string[]} urls - Array of candidate URLs for the target file
 * @param {string} currentUrl - Current page URL for context
 * @returns {string} Best matching URL
 */
function findClosestUrl(urls, currentUrl) {
    ...
    for (const url of urls) {
        ...
        if (sharedLength > bestScore) {
            bestScore = sharedLength;
            bestUrl = url;
        }
    }
...
function transformLinkPath(originalHref, env, sitemapData) {
    ...
    // Skip external links, absolute paths, and most file extensions
    // But process .ini and .zip files to convert them to absolute paths
    if (originalHref.startsWith('http://') ||
        originalHref.startsWith('https://') ||
        originalHref.startsWith('mailto:') ||
        originalHref.startsWith('/') ||
        /\.(png|jpg|jpeg|gif|svg|pdf)$/i.test(originalHref)) {
        return originalHref;
    }

    // Convert .ini and .zip files to absolute paths from root
    if (/\.(zip|ini)$/i.test(originalHref)) {
    ...
    const md2urlsToUse = sitemapData.md2urls || {};
    const currentUrl = (env && env._currentUrl) || '';
    ...
    if (baseTarget.endsWith('.md')) {
        ...
        if (mdTarget.startsWith('../')) {
            ...
            if (env && env.page && env.page.inputPath) {
                ...
                if (md2urlsToUse[resolvedPath]) {
                    mappedUrl = findClosestUrl(md2urlsToUse[resolvedPath], currentUrl);
                } else if (md2urlToUse[resolvedPath]) {
        ...
        if (!mappedUrl && md2urlsToUse[mdTarget]) {
            mappedUrl = findClosestUrl(md2urlsToUse[mdTarget], currentUrl);
        } else if (!mappedUrl && md2urlToUse[mdTarget]) {
        ...
        if (!mappedUrl && currentSection && !baseTarget.startsWith('../')) {
            ...
            if (md2urlsToUse[sectionFile]) {
                mappedUrl = findClosestUrl(md2urlsToUse[sectionFile], currentUrl);
            } else if (md2urlToUse[sectionFile]) {
        ...
        if (!mappedUrl) {
            ...
            if (currentSection) {
                for (const [mappedFile, mappedUrls] of Object.entries(md2urlsToUse)) {
                    const mappedBasename = mappedFile.split('/').pop();
                    if (mappedBasename === basename && mappedFile.startsWith(`${currentSection}/`)) {
                        mappedUrl = findClosestUrl(mappedUrls, currentUrl);
                        break;
                    }
                }
            }
...
function transformImagePath(originalPath, options = {}) {
    ...
    if (originalPath.startsWith('http://') ||
        ...
        /^\/_i[a-f0-9]{16}$/i.test(originalPath) ||
...
function applyRussianTypography(text) {
    ...
    // Pattern 5: Fix case where we have em dash + nbsp -> em dash + regular space
    // This handles cases where markdown already has em dash + nbsp
...
function createMarkdownInstance(options = {}) {
    ...
    const md = markdownIt({
        ...
        linkify: true,
        xhtmlOut: false,
        // Disable URL encoding for Cyrillic characters in links
        encodeEmails: false,
    });

    // Override normalizeLink to prevent encoding of Cyrillic characters
    const originalNormalizeLink = md.normalizeLink;
    md.normalizeLink = function(url) {
        // Keep original URL for Cyrillic characters and anchors
        if (url.startsWith('#') || /[\u0400-\u04FF]/.test(url)) {
            return url;
        }
        // Use default normalization for other URLs
        return originalNormalizeLink.call(this, url);
    };
    ...
    // Gallery support
    md.use(require('./markdown-gallery'), {
        imageMapping: options.imageMapping,
    });

    // Grid support
    md.use(require('./markdown-grid'));

    // Partner link support
    md.use(require('./markdown-partner-link'));

    // Add typography plugin before other processing
    md.use(function(md) {
        md.core.ruler.before('normalize', 'russian_typography', function(state) {
            // Apply typography to the source text before tokenization, but skip all code
            let src = state.src;

            // Extract code blocks (both fenced and indented) and inline code, protect them from typography
            const codeBlocks = [];
            let processedSrc = src;

            // Match HTML code blocks FIRST (they have priority over everything)
            const htmlCodeRegex = /<pre[^>]*>[\s\S]*?<\/pre>/g;
            processedSrc = processedSrc.replace(htmlCodeRegex, (match, _offset) => {
                const placeholder = `__CODE_BLOCK_${codeBlocks.length}__`;
                codeBlocks.push({ placeholder, original: match });
                return placeholder;
            });

            // Match fenced code blocks NEXT (they have priority over inline code)
            // Must match fences with optional leading whitespace (for indented code in lists)
            const fencedCodeRegex = /^( {0,3})```[\s\S]*?```/gm;
            processedSrc = processedSrc.replace(fencedCodeRegex, (match, _offset) => {
                const placeholder = `__CODE_BLOCK_${codeBlocks.length}__`;
                codeBlocks.push({ placeholder, original: match });
                return placeholder;
            });

            // Match indented code blocks (4+ spaces at start of line)
            const indentedCodeRegex = /^(?: {4}[^\n]*\n?)+/gm;
            processedSrc = processedSrc.replace(indentedCodeRegex, (match, _offset) => {
                const placeholder = `__CODE_BLOCK_${codeBlocks.length}__`;
                codeBlocks.push({ placeholder, original: match });
                return placeholder;
            });

            // Match inline code LAST (after removing fenced blocks to avoid conflicts)
            const inlineCodeRegex = /`[^`]+`/g;
            processedSrc = processedSrc.replace(inlineCodeRegex, (match, _offset) => {
                const placeholder = `__CODE_BLOCK_${codeBlocks.length}__`;
                codeBlocks.push({ placeholder, original: match });
                return placeholder;
            });

            // Apply typography to the text without any code
            processedSrc = applyRussianTypography(processedSrc);

            // Restore original code blocks and inline code
            codeBlocks.forEach(({ placeholder, original }) => {
                processedSrc = processedSrc.replace(placeholder, original);
            });
    ...
    md.renderer.rules.heading_open = function(tokens, idx, _options, env) {
        ...
        if (env._processingState && env._currentUrl) {
            ...
            // Store only the decoded version - we don't want encoded anchors in HTML
    ...
    md.renderer.rules.link_open = function(tokens, idx, _options, env, _renderer) {
        ...
        if (href) {
            ...
            if (href.endsWith('.md') || href.includes('.md#') || href.includes('.md?')) {
                ...
                if (newHref.includes('#') && env._processingState && env._currentUrl) {
                    ...
                    if (anchorMapping) {
                        // Try direct match first, then decode if needed
                        let ourAnchor = anchorMapping.get(githubAnchor);
                        if (!ourAnchor) {
                            try {
                                const decodedAnchor = decodeURIComponent(githubAnchor);
                                ourAnchor = anchorMapping.get(decodedAnchor);
                            } catch (error) {
                                // If decoding fails, we already tried direct match
                            }
                        }
                ...
                // For .ini and .zip files, ensure they have absolute paths
                if (/\.(zip|ini)$/i.test(href) && !href.startsWith('/')) {
                    transformedHref = `/${href}`;
                }

                // Always apply asset transformation for GitHub links
                const assetTransformedHref = transformAssetLinkPath(href);
                if (assetTransformedHref !== href) {
                    transformedHref = assetTransformedHref;
                }
        ...
        // Check if we need to add a space before the link
        // Look at the previous token to see if it's text content
        const prevToken = tokens[idx - 1];
        let linkHtml = `<a ${buildAttributesWithClassFirst(attrs)}>`;

        if (prevToken && prevToken.type === 'text' && prevToken.content) {
            // If the previous text ends with a letter, number, or common symbol, add a space
            // This handles cases like "Windows +", "Option -", etc.
            if (/[a-zA-Zа-яА-ЯёЁ0-9+\-×÷=<>]$/.test(prevToken.content)) {
                linkHtml = ' ' + linkHtml;
            }
        }
    ...
    // Typography is handled by the core plugin, so we don't need to apply it here
    // Just use the original renderers
    ...
    md.renderer.rules.code_block = function(tokens, idx, _options, _env, _renderer) {
        ...
        token.content = transformGitHubLinksInText(token.content);
        ...
        return `<pre ${formatAttribute('class', 'article__code')}><code>${token.content}</code>` +
            `${copyBtn}</pre>`;
    };
    ...
    // Custom renderer for inline code to transform asset URLs and add CSS class
    md.renderer.rules.code_inline = function(tokens, idx, options, env, renderer) {
        const token = tokens[idx];
        token.content = transformGitHubLinksInText(token.content);

        // Add article__inline-code class to inline code elements
        const existingClass = token.attrGet('class') || '';
        const newClass = existingClass ? `${existingClass} article__inline-code` : 'article__inline-code';
    ...
    md.render = function(src, env, renderOptions = {}) {
        ...
        // Enhanced minification while preserving code blocks and inline code
        // Extract code blocks and inline code to protect them from minification
        const codeBlocks = [];
        ...
        // Also replace inline code to protect it from minification
        processedHtml = processedHtml.replace(/<code[^>]*>[\s\S]*?<\/code>/g, (match) => {
            const placeholder = `__CODE_BLOCK_${codeBlocks.length}__`;
            codeBlocks.push(match);
            return placeholder;
        });

        // Apply comprehensive minification to HTML without code blocks
        // eslint-disable-next-line max-len
        const structuralBlockRegex = /<\/(div|ul|ol|section|article|header|footer|nav|aside|main|p|h[1-6])>\s+<(div|ul|ol|section|article|header|footer|nav|aside|main|p|h[1-6])/gi;
        processedHtml = processedHtml
            // Remove newlines and spaces between block-level tags, but preserve spaces between text and inline elements
            .replace(/>\s+</g, (match, offset, string) => {
                // Look at the context around this match to determine if we should preserve space
                const beforeMatch = string.substring(0, offset);
                const afterMatch = string.substring(offset + match.length);

                // Check if there's text content immediately before the closing tag
                // Pattern: ...text</tag><tag>... or ...text</tag><tag>text...
                const hasTextBefore = /[^>][^<]*$/.test(beforeMatch.slice(-20));
                const hasTextAfter = /^[^<]*[^<]/.test(afterMatch.slice(0, 20));

                // Check if we're dealing with inline elements
                const prevTag = beforeMatch.match(/<\/([a-z]+)>[^<]*$/);
                const nextTag = afterMatch.match(/^<([a-z]+)/);

                const inlineElements = new Set([
                    'a', 'span', 'strong', 'em', 'b', 'i', 'u', 'code', 'small', 'sub', 'sup',
                    'mark', 'abbr', 'time', 'data', 'cite', 'q', 'dfn', 'var', 'samp', 'kbd',
                    's', 'del', 'ins',
                ]);

                const prevIsInline = prevTag && inlineElements.has(prevTag[1]);
                const nextIsInline = nextTag && inlineElements.has(nextTag[1]);

                // Special case: always preserve space between code and link elements
                if (prevTag && prevTag[1] === 'code' && nextTag && nextTag[1] === 'a') {
                    return '> <';
                }

                // Special case: always preserve space between link and code elements
                if (prevTag && prevTag[1] === 'a' && nextTag && nextTag[1] === 'code') {
                    return '> <';
                }

                // Special case: preserve space after colon before inline elements
                // Pattern: "text: <strong>...</strong>" or "text: <em>...</em>"
                const hasColonBefore = /:[\s]*$/.test(beforeMatch.slice(-10));
                if (hasColonBefore && nextIsInline) {
                    return '> <';
                }

                // Preserve space if:
                // 1. There's text before/after and we're dealing with inline elements
                // 2. Either side is an inline element (to preserve readability)
                if ((hasTextBefore && nextIsInline) ||
                    (hasTextAfter && prevIsInline) ||
                    (prevIsInline && nextIsInline)) {
                    return '> <';
                }

                // Otherwise, remove all whitespace
                return '><';
            })
            // Remove newlines and extra spaces after opening tags (except in pre/code)
            .replace(/>(?![^<]*<\/(?:pre|code)>)[\s\n]+/g, '>')
            // Remove newlines and extra spaces before closing tags (except in pre/code)
            .replace(/[\s\n]+<(?!\/(?:pre|code)>)/g, '<')
            // Remove newlines between major structural blocks
            .replace(structuralBlockRegex, '</$1><$2')
        ...
        // Fix case where space was already removed between colon and inline element
        // Pattern: "text:<strong>" -> "text: <strong>"
        processedHtml = processedHtml.replace(
            /:<(strong|em|b|i|code|a|span|s|del|ins|mark|small|sub|sup|kbd|var|samp|abbr|time|data|cite|q|dfn)([>\s])/g,
            ': <$1$2',
        );
        ...
        codeBlocks.forEach((codeBlock, index) => {
            ...
            // Special handling for inline code: always ensure space after code if followed by link
            if (codeBlock.match(/^<code[^>]*>/) && !codeBlock.includes('<pre')) {
                const afterPlaceholder = processedHtml.split(placeholder)[1] || '';
                // Check if the next element starts with <a (link)
                if (afterPlaceholder.trim().startsWith('<a')) {
                    // Add space after the code element
        ...
        // Additional aggressive cleanup for remaining newlines between block elements (after code blocks restored)
        processedHtml = processedHtml
            .replace(/<\/div>\s+<div/g, '</div><div')
            .replace(/<\/ul>\s+<div/g, '</ul><div')
            .replace(/<\/footer>\s+<div/g, '</footer><div')
            // Final cleanup: remove newlines between closing and opening tags, but be more careful
            // Remove the aggressive cleanup that was removing spaces between code and links
            .replace(
                new RegExp(
                    '(</(?:div|ul|ol|li|p|h[1-6]|section|article|header|footer|nav|aside|main|' +
                    'blockquote|table|thead|tbody|tr|th|td)>)\\s+(?=<)',
                    'g',
                ),
                '$1',
            );
        ...
        // Apply Russian typography to HTML content (after minification)
        // This handles typography patterns that span across HTML elements
        processedHtml = processedHtml
            // Pattern: link + dash + text -> link + nbsp + dash + space + text
            .replace(/<\/a>—/g, '</a>\u00A0— ')
            // Pattern: text + dash + link -> text + nbsp + dash + space + link
            .replace(/([a-zA-Zа-яА-ЯёЁ])—(?=<a)/g, '$1\u00A0— ')
            // Pattern: link + space + dash + space + text -> link + nbsp + dash + space + text
            .replace(/<\/a> +— +/g, '</a>\u00A0— ')
            // Pattern: text + space + dash + space + link -> text + nbsp + dash + space + link
            .replace(/([a-zA-Zа-яА-ЯёЁ]) +— +(?=<a)/g, '$1\u00A0— ')
            // Add spaces between text and inline elements for readability (only when not typography patterns)
            // Add space after links if followed by text, but NOT if followed by dash, nbsp, or already has space
            .replace(/<\/a>(?=[a-zA-Zа-яА-ЯёЁ0-9(«"„[])/g, '</a> ')
            // Add space before links if preceded by text, but NOT if preceded by dash or nbsp
            .replace(/([a-zA-Zа-яА-ЯёЁ0-9+\-×÷=<>)»"\],])(?=<a)/g, '$1 ')
            // Add spaces between text and inline elements (strong, em, b, i, etc.)
            // Add space after inline elements if followed by letter/number/bracket
            // eslint-disable-next-line max-len
            .replace(/<\/(strong|em|b|i|u|s|del|ins|mark|small|sub|sup|abbr|time|data|cite|q|dfn|var|samp|kbd)>(?=[a-zA-Zа-яА-ЯёЁ0-9(«"„[])/g, '</$1> ')
            // Add space before inline elements if preceded by letter/number/bracket
            // eslint-disable-next-line max-len
            .replace(/([a-zA-Zа-яА-ЯёЁ0-9+\-×÷=<>)»"\],])(?=<(strong|em|b|i|u|s|del|ins|mark|small|sub|sup|abbr|time|data|cite|q|dfn|var|samp|kbd)>)/g, '$1 ')
            // Fix double spaces and nbsp after dash - should be regular space
            .replace(/\u00A0—  +/g, '\u00A0— ')
            .replace(/\u00A0—\u00A0/g, '\u00A0— ')
            // Fix case where we have nbsp + dash + nbsp (should be nbsp + dash + space)
            .replace(/\u00A0—\u00A0/g, '\u00A0— ')
            // Fix spaces after links when typography already added nbsp (remove extra spaces)
            .replace(/<\/a>\s+\u00A0—/g, '</a>\u00A0—')
            // Fix double spaces and nbsp after dash - should be regular space
            .replace(/\u00A0—  +/g, '\u00A0— ')
            .replace(/\u00A0—\u00A0/g, '\u00A0— ');
