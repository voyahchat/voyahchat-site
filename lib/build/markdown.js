...
function createMarkdownInstance(options = {}) {
    ...
    md.renderer.rules.text = function(tokens, idx, options, env, renderer) {
        ...
        // Apply Russian typography rules to text content, but skip if inside code
        if (token.content) {
            // Check if this text token is inside a code block or inline code
            // We need to look at the token hierarchy to determine context
            let isInCode = false;

            // Check parent tokens to see if we're inside code
            for (let i = idx - 1; i >= 0; i--) {
                const prevToken = tokens[i];
                if (prevToken.type === 'code_block' || prevToken.type === 'fence') {
                    isInCode = true;
                    break;
                }
                // Stop checking if we hit a block boundary
                if (prevToken.type.endsWith('_open') || prevToken.type.endsWith('_close')) {
                    break;
                }
            }

            // Also check if we're in an inline code token by looking at the current token's level
            // and checking for code_inline tokens at the same level
            if (!isInCode) {
                for (let i = 0; i < tokens.length; i++) {
                    const checkToken = tokens[i];
                    if (checkToken.type === 'code_inline' &&
                        Math.abs(checkToken.level - token.level) <= 1) {
                        // Check if this text token is part of the inline code
                        const inlineStart = Math.min(i, idx);
                        const inlineEnd = Math.max(i, idx);
                        let foundCode = false;

                        for (let j = inlineStart; j <= inlineEnd; j++) {
                            if (tokens[j].type === 'code_inline') {
                                foundCode = true;
                                break;
                            }
                        }
    ...
    md.renderer.rules.inline = function(tokens, idx, options, env, renderer) {
        ...
        // Apply typography to all child tokens, but skip inline code tokens themselves
        if (token.children) {
            token.children.forEach(child => {
                if (child.type === 'text' && child.content) {
                    // Apply typography to text tokens (but not to code_inline tokens)
                    child.content = applyRussianTypography(child.content);
                }
                // Note: code_inline tokens are handled by their own renderer and should not get typography
            });
        }
