...
test('build-timestamps - NO files should have build-time timestamps', async (t) => {
    ...
    for (const file of allFiles) {
        ...
        // Skip timestamps.json and timestamps-server.json as they're generated during build
        // Also skip .htaccess as its timestamp is not important for our build process
        if (relativePath === 'timestamps.json' || relativePath === 'timestamps-server.json' ||
            relativePath === 'new.pdf' || relativePath === '.htaccess') {
            continue;
        }
        ...
        if (relativePath.endsWith('.zip') || relativePath.endsWith('.pdf')) {
            try {
                ...
                if (content.includes('# Placeholder file for') && content.includes('likely due to LFS limits in CI')) {
                    isPlaceholder = true;
                }
            } catch {
                // If we can't read the file, continue with normal checks
            }
        }
