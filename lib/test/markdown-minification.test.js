...
// Test 7: Preserve space between link and guillemet quotes
test('Minification - preserve space between link and guillemet quotes', (t) => {
    const sitemap = createMockSitemap({
        pages: [
            {
                url: '/test',
    ...
    const markdown = '- [Обновления](test.md) «по воздуху»\n';
    const html = md.render(markdown, {});

    // Между </a> и «по должен быть пробел
    t.true(
        html.includes('</a> «по'),
        `Space before guillemet lost: ${html}`,
    );

    // Не должно быть слипания
    t.false(
        html.includes('</a>«'),
        `Link stuck to guillemet: ${html}`,
    );
});

// Test 8: Preserve space between guillemet and link
test('Minification - preserve space between guillemet and link', (t) => {
    const sitemap = createMockSitemap({
        pages: [
            {
                url: '/test',
    ...
    const markdown = 'текст» [ссылка](test.md) продолжение\n';
    const html = md.render(markdown, {});

    t.true(
        html.includes('» <a'),
        `Space after closing guillemet lost: ${html}`,
    );
});

// Test 9: Preserve space after + symbol before link
test('Minification - preserve space after + symbol before link', (t) => {
    const sitemap = createMockSitemap({
        pages: [
            {
                url: '/cable',
    ...
    const markdown = '- Windows + [кабель](cable.md)\n';
    const html = md.render(markdown, {});

    // Должен быть пробел между + и <a
    t.true(
        html.includes('+ <a'),
        `Space after + lost: ${html}`,
    );

    // Не должно быть слипания
    t.false(
        html.includes('+<a'),
        `Plus symbol stuck to link: ${html}`,
    );
});

// Test 10: Preserve space after - symbol before link
test('Minification - preserve space after - symbol before link', (t) => {
    const sitemap = createMockSitemap({
        pages: [
            {
    ...
    const markdown = '- Option - [choice](test.md)\n';
    const html = md.render(markdown, {});
    ...
    t.false(
        html.includes('-<a'),
        `Dash symbol stuck to link: ${html}`,
    );
});

// Test 11: Preserve space between comma and link
test('Minification - preserve space between comma and link', (t) => {
    const sitemap = createMockSitemap({
        pages: [
            {
                url: '/test',
    ...
    t.true(
        html.includes(', <a'),
        `Space after comma lost: ${html}`,
    );
    t.false(
        html.includes(',<a'),
        `Comma stuck to link: ${html}`,
    );
});
