...
// Test 6: Converts HTML paths to flat filenames and handles hash-prefixed files
test('CompressionBuilder() - converts HTML paths to flat filenames', (t) => {
    ...
    // Hash-prefixed files should remain as-is
    t.is(builder.getFlatFilename('_c9008915a30a21706'), '_c9008915a30a21706');
    t.is(builder.getFlatFilename('_j904685a5dab014e8'), '_j904685a5dab014e8');
...
// Test: Finds and compresses hash-prefixed CSS files
test('CompressionBuilder() - finds and compresses hash-prefixed CSS files', async (t) => {
    const cssContent = '.test { color: red; margin: 10px; }';
    await fs.writeFile(path.join(t.context.siteDir, '_c9008915a30a21706'), cssContent);

    const builder = createTestBuilder(t);
    const files = await builder.findCompressibleFiles();

    t.true(files.includes('_c9008915a30a21706'), 'Should find hash-prefixed CSS file');

    await builder.ensureDirectories();
    const stats = { add: () => {}, files: [] };
    await builder.compressFile('_c9008915a30a21706', stats);

    // Verify compressed files were created
    const brotliPath = path.join(t.context.brotliDir, '_c9008915a30a21706.br');
    const gzipPath = path.join(t.context.gzipDir, '_c9008915a30a21706.gz');
...
// Test: Finds and compresses hash-prefixed JS files
test('CompressionBuilder() - finds and compresses hash-prefixed JS files', async (t) => {
    const jsContent = 'const x=1;console.log(x);';
    await fs.writeFile(path.join(t.context.siteDir, '_j904685a5dab014e8'), jsContent);

    const builder = createTestBuilder(t);
    const files = await builder.findCompressibleFiles();

    t.true(files.includes('_j904685a5dab014e8'), 'Should find hash-prefixed JS file');

    await builder.ensureDirectories();
    const stats = { add: () => {}, files: [] };
    await builder.compressFile('_j904685a5dab014e8', stats);

    // Verify compressed files were created
    const brotliPath = path.join(t.context.brotliDir, '_j904685a5dab014e8.br');
    const gzipPath = path.join(t.context.gzipDir, '_j904685a5dab014e8.gz');
...
// Test: Hash-prefixed pattern detection
test('CompressionBuilder() - detects hash-prefixed files correctly', (t) => {
    const builder = new CompressionBuilder();

    // Valid hash-prefixed files
    t.true(builder.hashPrefixPattern.test('_c9008915a30a21706'));
    t.true(builder.hashPrefixPattern.test('_j904685a5dab014e8'));
    t.true(builder.hashPrefixPattern.test('_i0e5f33c7435efacf'));

    // Invalid patterns
    t.false(builder.hashPrefixPattern.test('_c90089')); // Too short
    t.false(builder.hashPrefixPattern.test('_x9008915a30a21706')); // Wrong prefix
    t.false(builder.hashPrefixPattern.test('c9008915a30a21706')); // Missing underscore
    t.false(builder.hashPrefixPattern.test('_c9008915a30a2170g')); // Invalid hex char
