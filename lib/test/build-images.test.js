...
test('ImageProcessor() - should constructor with default options', (t) => {
    ...
    t.deepEqual(processor.supportedFormats, ['.png', '.jpg', '.jpeg', '.gif', '.webp']);
    t.is(processor.svgFormat, '.svg');
...
test('ImageProcessor() - should processImage with cache hit', async (t) => {
    ...
    processor.cache = {
        'test.png': {
            ...
            hash: 'existing-hash',
            hashedFilename: '_iexisting-hash',
            imageRef: '_iexisting-hash',
        },
    };
    ...
    // Should preserve existing mapping with _i prefix
    t.true(processor.imageMapping.has('test.png'));
    t.is(processor.imageMapping.get('test.png'), '_iexisting-hash');
...
test('ImageProcessor() - should processImage with cache miss', async (t) => {
    ...
    if (processor.stats.filesProcessed > 0) {
        t.true(processor.imageMapping.has('test.png'));
        const imageRef = processor.imageMapping.get('test.png');

        // imageMapping stores the imageRef (_i{hash}), not the hashedFilename
        t.true(imageRef.startsWith('_i'));

        t.truthy(processor.cache['test.png']);
        // Cache stores the actual hashedFilename with extension
        const hashedFilename = processor.cache['test.png'].hashedFilename;
        t.true(hashedFilename.endsWith('.png'));
        t.is(processor.cache['test.png'].imageRef, imageRef);
    }
...
test('ImageProcessor() - should processImage handles duplicate content', async (t) => {
    ...
    // Only one file should be created - extract actual hash from imageRef (_i prefix)
    const actualHash = hash1.replace(/^_i/, '');
    const ext = path.extname('test1.png');
    const hashedFilename = `${actualHash}${ext}`;
    // Files are stored in subdirectories based on extension
    const outputFile = path.join(testDir, 'png', hashedFilename);
...
test('ImageProcessor() - should generateMappingFile creates correct structure with _i prefix', async (t) => {
    const testDir = t.context.testDir;
    ...
    processor.imageMapping = new Map([
        ['images/test.png', '_iabc123def456'],
    ...
    // Mapping should use _i prefix for raster images, hash.svg for SVG
...
test('ImageProcessor() - should build process completes successfully', async (t) => {
    ...
    await Promise.all(Object.values(result.mapping).map(async (imageRef) => {
        // Extract actual hash from imageRef (_i or _s prefix)
        const actualHash = imageRef.replace(/^_[is]/, '');
        // Find the corresponding hashedFilename from imageStats
        const hashedFilename = Array.from(result.imageStats.keys()).find(key => key.includes(actualHash));
        if (hashedFilename) {
            ...
            if (ext === '.png') {
                ...
                subdir = 'jpg';
            } else if (ext === '.svg' || hashedFilename.startsWith('_s')) {
                subdir = 'svg';
            } else if (ext === '.webp') {
                subdir = 'webp';
            }
            const outputPath = path.join(processor.outputDir, subdir, hashedFilename);
...
test('ImageProcessor() - should generates correct image mapping with _i prefix', async (t) => {
    const testDir = t.context.testDir;
    const buildDir = path.join(testDir, '.build');
    ...
    processor.buildDir = buildDir;
    processor.imageMapping = new Map([
        ['path/to/image1.png', '_ihash1'],
        ['path/to/image2.jpg', '_ihash2'],
    ...
    t.deepEqual(mappingContent, {
        ...
        'path/to/image2.jpg': '_ihash2',
        'image3.svg': 'hash3.svg',
    }, 'Mapping should contain raster images with _i prefix and SVG with hash.svg');
...
test('ImageProcessor() - should handles too many open files gracefully', async (t) => {
    ...
    // Should handle many files gracefully (some may fail, but no crashes)
    const filesProcessed = processor.stats.filesProcessed;
    t.true(filesProcessed > 0, 'Should process some images without EMFILE crashes');
    t.true(filesProcessed + processor.stats.errors === imageCount, 'Should attempt to process all images');
});
...
test('ImageProcessor() - should handles disk space errors gracefully', async (t) => {
    ...
    // Mock the processor's internal method to simulate ENOSPC error
    // This avoids affecting the global fs.copyFile used by copyFixture()
    const originalProcessImage = processor.processImage;
    processor.processImage = async function(imagePath) {
        // Call the original method first to set up everything
        await originalProcessImage.call(this, imagePath);

        // If we get here without errors, simulate the ENOSPC error during file writing
        // by checking if a file copy operation would have occurred
        if (this.stats.filesProcessed > 0 && this.stats.errors === 0) {
            this.stats.errors = 1;
            this.stats.filesProcessed--; // Rollback the processed count since file write failed
    ...
    try {
        ...
        // Expected to throw due to ENOSPC simulation
        t.is(processor.stats.errors, 1, 'Should count disk space error');
        t.true(error.message.includes('ENOSPC'), 'Should throw ENOSPC error');
    }
});
