...
const { MEDIA_EXTENSIONS, BYTES_PER_MB, OUTPUT_DIRS, OUTPUT_FILES, SCRAPER_DIRS } = require('./constants');
...
class TelegramDownloader {
    ...
    async init() {
        ...
        // Suppress ALL TelegramClient logs completely
        if (!this.verbose) {
            console.info = () => {}; // Completely suppress all info logs
            console.log = () => {}; // Also suppress console.log from Telegram client
        }

        const downloadConfig = await this.getDownloadConfig();

        // Create a custom logger that filters out chunk download messages
        const customLogger = this.verbose ? {
            info: (...args) => {
                ...
                if (typeof message === 'string' &&
                    ...
                    return; // Filter out all Telegram client connection messages
                }
                console.info(...args);
            },
            warn: console.warn,
            ...
            info: () => {}, // Suppress all info logs in non-verbose mode
            warn: () => {},
            error: console.error, // Still show errors
            debug: () => {},
        };
        ...
        await this.client.start({
            ...
            onError: (err) => {
                ...
                if (err.errorMessage === 'API_ID_INVALID') {
                    this.error('Invalid API credentials. Check api_id and api_hash in config/auth-telegram.yml');
                    this.error('Get API credentials from https://my.telegram.org/apps');
                    process.exit(1);
                }
    ...
    async getChat() {
        ...
        try {
            ...
            if (err.message.includes('flood') || err.message.includes('FLOOD_WAIT')) {
                const waitTime = this.extractFloodWaitTime(err.message);
     ...
     * @param {string} errorMessage - Error message containing flood wait time
     ...
     */
    async downloadMedia(message, sectionDir, sectionStats = null) {
        ...
        try {
            ...
            if (sectionStats) {
                sectionStats.addCachedBytes(stats.size);
            }
        ...
        try {
            this.log(`  Downloading media ${fileName} (${Math.round(fileSizeBytes / BYTES_PER_MB)}MB)...`);

            // Optimize download options for better performance
            const downloadOptions = {
                workers: 1, // Use single worker for better reliability
                progressCallback: fileSizeBytes > 10 * BYTES_PER_MB ? (progress) => {
            ...
            buffer = await executeWithTimeoutAndRetry(
                () => this.client.downloadMedia(message, downloadOptions),
                timeoutMs,
                {
            ...
            if (err.message === 'not implemented') {
                this.log(`  Media type not supported: ${message.media.className}`);
                this.stats.incrementMedia('skipped');
                return [];
            }
    ...
    async messageToJson(message, sectionDir, sectionStats = null) {
        ...
        if (message.media) {
            json.media = await this.downloadMedia(message, sectionDir, sectionStats);
        }
    ...
    async downloadSection(section) {
        ...
        try {
            ...
            const linksStats = await this.getExternalLinksStats(sectionDir);
            const linksInfo = [];
            ...
            await this.calculateSectionSize(sectionDir, sectionStats);
                    ...
                    try {
                        ...
                        try {
                            ...
                            // Continue to download missing messages
                                ...
                                const cachedLinksStats = await this.getExternalLinksStats(sectionDir);
            ...
            const finalLinksStats = await this.getExternalLinksStats(sectionDir);
        ...
        try {
            ...
            try {
                ...
                if (messageFiles.length === metadata.messageCount) {
                    ...
                    // Add external links stats
                    const linksStats = await this.getExternalLinksStats(channelDir);
                    if (linksStats.totalLinks > 0) {
                    ...
                    // Calculate accurate section size using du command
                    await this.calculateSectionSize(channelDir, overallStats);
                    ...
                    for (const message of messages) {
                        ...
                        try {
                            ...
                            try {
                                ...
                                await this.scrapeExternalLinks(messageJson, channelDir, message.id);
            ...
            // Calculate accurate section size using du command
            await this.calculateSectionSize(channelDir, sectionStats);
    ...
    /**
     * Get external links statistics for a channel
     * @param {string} channelDir - Channel directory path
     * @returns {Promise<Object>} Statistics object with counts and sizes
     */
    async getExternalLinksStats(channelDir) {
        const linksDir = path.join(channelDir, SCRAPER_DIRS.links);
        const stats = {
            totalLinks: 0,
            downloadedLinks: 0,
            skippedLinks: 0,
     ...
     * Downloads linked pages and images using wget
     * @param {Object} messageJson - Parsed message JSON
     * @param {string} channelDir - Channel directory path
     * @param {number} messageId - Message ID for organizing output
     * @returns {Promise<void>}
     */
    async scrapeExternalLinks(messageJson, channelDir, messageId) {
        const externalUrls = this.scraper.extractExternalUrls(messageJson.text);
        if (externalUrls.length === 0) {
            return;
        }

        this.log(`  Found ${externalUrls.length} external link(s) in message ${messageId}`);

        const linksDir = path.join(channelDir, SCRAPER_DIRS.links, String(messageId));

        for (const url of externalUrls) {
            try {
                const result = await this.scraper.downloadPage(url, linksDir);
                if (result.skipped) {
                    this.log(`  Skipped ${url}: ${result.reason}`);
                } else if (result.success) {
                    this.log(`  Downloaded ${url}`);
                } else {
                    this.log(`  Failed to download ${url}: ${result.error}`);
                }
            } catch (err) {
                this.log(`  Error scraping ${url}: ${err.message}`);
            }
        }
    }
     ...
     * @param {string} sectionDir - Section directory path
     * @param {SectionDownloadStatistics} sectionStats - Section statistics object
     * @returns {Promise<void>}
     */
    async calculateSectionSize(sectionDir, sectionStats) {
        try {
            await fs.access(sectionDir);
            const totalSize = await this.getDirectorySize(sectionDir);
            if (totalSize > 0) {
    ...
    /**
     * Get total size of a directory using du command for accuracy
    ...
    async getDirectorySize(dirPath) {
        ...
        try {
            // Use du command for accurate and fast directory size calculation
            // -s flag: summarize (total only)
            // -k flag: display in kilobytes
            const { stdout } = await execPromise(`du -sk "${dirPath}"`);
            // Extract size in kilobytes and convert to bytes
            const sizeKb = parseInt(stdout.split('\t')[0], 10);
            return sizeKb * 1024;
        } catch (err) {
            this.log(`Error using du command for ${dirPath}: ${err.message}`);
            return 0;
        }
    }
}
