...
// Inline Code Tests
test('markdown renderer - inline code has article__inline-code class', (t) => {
    const md = createMarkdownInstance();
    const result = md.render('`inline code`');

    t.true(result.includes('<code class="article__inline-code">inline code</code>'));
});

test('markdown renderer - inline code preserves trailing space', (t) => {
    const md = createMarkdownInstance();
    const result = md.render('`2026 `');

    t.true(result.includes('<code class="article__inline-code">2026 </code>'));
});

test('markdown renderer - inline code with special chars', (t) => {
    const md = createMarkdownInstance();
    const result = md.render('`<test>`');

    t.true(result.includes('<code class="article__inline-code">&lt;test&gt;</code>'));
});
